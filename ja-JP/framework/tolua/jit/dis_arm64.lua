...
function slot55(slot0)
	...
	slot10, slot11 = uv8(slot13, "^([a-z0-9]*)(.*)")
	...
	if uv9(slot11, 1, 1) == "." then
		slot19, slot11 = uv8(slot11, "^([a-z0-9.]*)(.*)")
		slot8 = slot8 .. slot16
	end

	if uv8(slot11, "[gf]") then
		if slot16 == "g" then
			slot12 = uv4(slot6, 2147483648.0) ~= 0 and uv10.x or uv10.w
		else
			slot12 = uv4(slot6, 4194304) ~= 0 and uv10.d or uv10.s
		end
	end
...
function slot60.create(slot0, slot1, slot2)
	slot3.code = slot0
...
function slot60.disass(slot0, slot1, slot2)
	slot3 = uv0(slot0, slot1, slot2)

	slot3:disass()
end

function slot60.regname(slot0)
	if slot0 < 32 then
		return uv0.x[slot0]
	end
