...
function slot0.moveNext(slot0, slot1)
	...
	slot8.id = slot2.id
	slot8.pos = slot2:getSurroundGrid()[slot1]
...
function slot0.clearInterAction(slot0, slot1)
	...
	if slot2:hasInterActionFurnitrue() then
...
function slot0.InterActionTransportAgain(slot0, slot1, slot2)
	...
	slot7 = nil
	slot11.shipId = slot1
	slot11.furnitureId = (#slot5 ~= 1 or slot5[1]) and _.detect(slot5, function (slot0)
		return slot0 ~= uv0
	end)
...
function slot0.addSpineInterAction(slot0, slot1, slot2)
	...
	if slot3:getPosition().x == slot4:getSpineAinTriggerPos().x and slot6.y == slot5.y then
		slot0:cancelShipMove(slot1)
		slot4:setSpineId(slot1)
		slot3:setSpineId(slot2)
		slot3:reduceRate()
		...
		if not slot7:isIllegalPos(slot5, slot2) then
...
function slot0.addMoveForFurniture(slot0, slot1, slot2)
	slot3 = slot0:getFurnitureById(slot1)
...
function slot0.addSpineExtraInterAction(slot0, slot1, slot2)
	...
	slot9.shipId = slot1
	slot9.furnitureId = slot2
	slot9.pos = slot0.data.furnitures[slot2]:addSpineExtra(slot1)
...
function slot0.clearSpineExtraInterAction(slot0, slot1, slot2)
	...
	slot9.shipId = slot1
	slot9.furnitureId = slot2
	slot9.pos = slot0.data.furnitures[slot2]:removeSpineExtra(slot1)
...
function slot0.setStageInteraction(slot0, slot1, slot2)
	...
	if slot3:hasInterActionFurnitrue() then
	...
	if slot5:getCanMoveNearerPosOnFurnitrue(slot1, slot2) then
...
function slot0.addMoveOnFurnitrue(slot0, slot1, slot2, slot3)
	if not BackYardConst.DEBUG then
		...
		slot0.timer[slot6] = Timer.New(function ()
			...
			slot5 = uv0
			slot8.id = uv1
			slot8.position = slot4
			slot8.isLastStep = uv7 <= uv5

			slot5.sendNotification(slot5, uv4.MOVE_ON_FURNTURE, {})
...
function slot0.clearStageInteraction(slot0, slot1)
	...
	if slot0.data.furnitures[slot2:getStageId()] then
...
function slot0.shipRomdonMove(slot0, slot1, slot2)
	...
	slot0.timer[slot3] = Timer.New(function ()
		...
		function ()
			for slot3, slot4 in ipairs(uv0) do
				if uv1.data:canMoveBoat(uv2, slot4) then
		...
		uv4 = uv4 + 1
		slot5 = slot0[math.random(1, #slot0)]

		uv0:moveShip(uv1, slot5, uv5)

		slot6 = uv0
		slot9.id = uv1
		slot9.pos = slot5
		slot9.isLastStep = uv6 <= uv4

		slot6.sendNotification(slot6, uv3.BACKYARD_SHIP_MOVE, {})
...
function slot0.randomInterAction(slot0, slot1, slot2)
	if uv0.isHappen(slot0.data.ships[slot1]:getRate()) then
		for slot8, slot9 in ipairs(slot2) do
			...
			if slot10:findInterActionFurnitrue(slot9, slot1) then
				slot0:addInterAction(slot1, slot10.id)

				return true
			elseif not slot3:inStageFurniture() then
				if slot0.data:findInterActionStage(slot9, slot1) then
...
function slot0.restoreFurnitures(slot0)
	...
	for slot7, slot8 in pairs(slot0.data:getSortFurnitures()) do
		if not slot0.data:isAddFurniture(slot8) then
			slot10 = slot0.data:isChangeFurniture(slot8)
		else
			slot10 = false
...
function slot0.changeFurniturePos(slot0, slot1, slot2, slot3)
	...
	slot9.furniture = Clone(slot0.data.furnitures[slot1])
	slot9.time = slot3 or 0
	slot9.prevPos = slot0.data.furnitures[slot1]:getPosition()
...
function slot0.replacePaper(slot0, slot1)
	if slot1:isWallPaper() then
		...
		slot6.prePaper = Clone(slot0.data.wallPaper)
		slot6.furniture = slot1
		slot6.type = Furniture.TYPE_WALLPAPER

		slot0:sendNotification(uv0.PAPER_REPLACE, {})
	else
		slot0.data:setFloorPaper(slot1)

		slot6.prePaper = Clone(slot0.data.floorPaper)
		slot6.furniture = slot1
		slot6.type = Furniture.TYPE_FLOORPAPER
...
function slot0.changeFurnitureDir(slot0, slot1, slot2)
	...
	slot6.furniture = slot0.data.furnitures[slot1]
...
function slot0.removeWallPaper(slot0)
	...
	slot5.prePaper = Clone(slot0.data.wallPaper)
	slot5.type = Furniture.TYPE_WALLPAPER
...
function slot0.removeFloorPaper(slot0)
	...
	slot5.prePaper = Clone(slot0.data.floorPaper)
	slot5.type = Furniture.TYPE_FLOORPAPER
