	...
	return ys.Battle.BattleBulletEmitter.New(function (slot0, slot1, slot2, slot3, slot4)
		...
		if uv0.equipSkinId > 0 then
			...
			if slot8:GetType() == ys.Battle.BattleConst.BulletType.CANNON or slot14 == slot15.BOMB then
				slot17[1] = EquipType.CannonQuZhu
				slot17[2] = EquipType.CannonQingXun
				slot17[3] = EquipType.CannonZhongXun
				slot17[4] = EquipType.CannonZhanlie
				slot17[5] = EquipType.CannonZhongXun2
...
function slot0.createEmitterAir(slot0, slot1, slot2, slot3)
	return ys.Battle.BattleBulletEmitter.New(function (slot0, slot1, slot2, slot3, slot4)
		...
		if uv2.equipSkinId > 0 then
			slot12[1] = EquipType.FighterAircraft
			slot12[2] = EquipType.TorpedoAircraft
			slot12[3] = EquipType.BomberAircraft

			if table.contains(pg.equip_skin_template[uv2.equipSkinId].equip_type, ({})[slot6.type]) then
...
function slot0.RemoveBullet(slot0, slot1, slot2)
	...
	if slot2 and slot3:GetMissFXID() and slot4 ~= "" then
		slot5 = slot0.seaFXPool
		slot9, slot6 = slot5:GetFX(slot4)
	...
	pg.TimeMgr.GetInstance():AddBattleTimer("barrageUpdateTimer", -1, 0.033, function ()
		for slot3 = #uv0.bulletList, 1, -1 do
			...
			elseif slot6.x < uv3 and slot5.x < 0 and slot4:GetType() ~= uv4.BulletType.BOMB then
				...
				if uv6 >= slot6.z or slot5.z <= 0 then
					if slot4._bulletData:IsOutRange(uv5) then
