...
function slot0.register(slot0)
	...
	slot9 = slot0.activityProxy

	if slot9:getActivityByType(ActivityConst.ACTIVITY_TYPE_BOSS_RANK) and not slot9:isEnd() then
		if getProxy(BillboardProxy):canFetch(PowerRank.TYPE_ACT_BOSS_BATTLE, slot9.id) then
			slot16.type = slot11
			slot16.activityId = slot12
...
function slot0.listNotificationInterests(slot0)
	...
	slot1[7] = GAME.ACTIVITY_BOSS_MSG_ADDED
	slot1[8] = GAME.ACT_BOSS_EXCHANGE_TICKET_DONE
	slot1[9] = GAME.GET_POWERRANK_DONE
