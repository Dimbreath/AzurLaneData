...
function slot0.OnInit(slot0)
	...
	slot1[1] = slot0.item1TF
	slot1[2] = slot0.item2TF
	slot0.itemTFList = {}
end
...
function slot0.OnUpdateFlush(slot0)
	...
	if checkExist(slot2:getConfig("config_client").story, {}, {
		...
		if not slot3:IsPlayed(slot2[slot0.nday][1]) then
	...
	for slot6 = 1, 2, 1 do
		...
		if not slot0.taskProxy:getTaskById(slot0.taskGroup[slot0.nday][slot6]) then
			slot11 = slot0.taskProxy:getFinishTaskById(slot10)
		end

		slot12 = slot11:getConfig("award_display")[1]
		slot13.type = slot12[1]
		slot13.id = slot12[2]
		slot13.count = slot12[3]
