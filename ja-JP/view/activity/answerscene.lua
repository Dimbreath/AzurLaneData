...
function slot0.didEnter(slot0)
	onButton(slot0, slot0.btnBack, function ()
		...
		if isActive(uv0.layerProcess) then
			slot2.content = i18n("answer_exit_tip")
	...
	onButton(slot0, slot0.btnHelp, function ()
		...
		slot2.helps = i18n("answer_help_tip")
...
function slot0.displaySelection(slot0)
	...
	for slot10 = 0, slot1:Find("subjects").childCount - 1, 1 do
		slot11 = slot3:GetChild(slot10)
		slot13 = slot6:isSubjectOpen(slot0.activity, slot10)

		setText(slot11:Find("score"), slot6:getScore(slot10) or "--")
...
function slot0.startProcess(slot0, slot1, slot2)
	...
	process = coroutine.create(function ()
		...
		slot6.configId = Answer.Ship1
		slot7.configId = Answer.Ship2
		slot6 = Ship.New({
			id = 2
		})
		...
		uv0:emit(AnswerMediator.ANSWER_SUBMIT, uv0.activity.id, uv14, _.map(uv4, function (slot0)
			slot1.id = slot0.id
			slot1.answer = slot0.select
			slot1.time = slot0.restTime
...
function slot0.displayResult(slot0, slot1, slot2, slot3)
	...
	process = coroutine.create(function ()
		...
		slot2.y = uv3.rect.height

		setAnchoredPosition(uv3, {})

		slot2.x = -uv4.rect.width

		setAnchoredPosition(uv4, {})

		slot2.x = uv5.rect.width
...
function slot0.willExit(slot0)
	...
	slot1 = slot0.layerProcess
	slot2 = slot0.layerProcess:Find("ship_right")

	if slot1:Find("ship_left").childCount > 1 then
		slot4.configId = Answer.Ship1
