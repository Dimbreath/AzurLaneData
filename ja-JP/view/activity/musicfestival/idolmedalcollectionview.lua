...
function slot0.initData(slot0)
	...
	for slot4 = 1, uv0.PAGE_NUM, 1 do
		slot0.pageIDList[slot4] = {}

		for slot8 = 1, uv0.MEDAL_NUM_PER_PAGE, 1 do
...
function slot0.addListener(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		slot2.helps = pg.gametip.music_collection.tip
...
function slot0.updateMedalView(slot0, slot1, slot2)
	...
	if slot5 then
		onButton(slot0, slot9, function ()
			slot3.id = uv0
			slot3.actId = ActivityConst.IDOL_MEDAL_COLLECTION
...
function slot0.isHaveActivableMedal()
	...
	if not slot0:getActivityById(ActivityConst.IDOL_MEDAL_COLLECTION) or slot0:isEnd() then
		return
	end
...
function slot0.checkAward(slot0)
	if #slot0.activeIDList == #slot0.allIDList and slot0.activityData.data1 ~= 1 then
		...
		slot4.activity_id = ActivityConst.IDOL_MEDAL_COLLECTION
