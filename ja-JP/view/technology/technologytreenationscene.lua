	...
	for slot10, slot11 in ipairs(pg.fleet_tech_template[slot3].add) do
		for slot18, slot19 in ipairs(slot11[2]) do
			if slot5[slot19] then
				slot22.attr = slot11[3]
				slot22.value = slot11[4]
...
function slot0.updateDetailPanel(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if slot2 == #pg.fleet_tech_group[slot3].techs and slot5 == false then
		setActive(slot0:findTF("Toggle", slot1), false)
	end
			...
			for slot23, slot24 in ipairs(slot16[2]) do
				...
				if slot2 then
					if not table.indexof(slot5, slot24, 1) then
						slot26.attr = slot16[3]
						slot26.value = slot16[4]
						slot26.attrColor = slot8
						slot26.valueColor = slot8
						slot25 = {}
					elseif not table.indexof(slot6[slot24], slot17, 1) then
						slot26.attr = slot17
						slot26.value = slot18
						slot26.attrColor = slot8
						slot26.valueColor = slot8
						slot25 = {}
					elseif slot18 ~= slot7[slot24][slot17] then
						slot26.attr = slot17
						slot26.value = slot18
						slot26.valueColor = slot8
						slot25 = {}
...
function slot0.upBuffList(slot0, slot1, slot2)
	...
	slot4:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setText(slot3:findTF("AttrText", slot2), AttributeType.Type2Name(pg.attribute_info_by_type[uv1[slot1 + 1].attr].name))
