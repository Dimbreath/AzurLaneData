...
function slot0.openSelectSkinPanel(slot0)
	...
	for slot5, slot6 in ipairs(slot0.skins) do
		slot7 = slot0.skinContainer

		if not slot0.skinCardMap[slot7:GetChild(slot5 - 1)] then
			slot0.skinCardMap[slot7] = ShipSkinCard.New(slot7.gameObject)
		end

		if slot0.shipVO:getRemouldSkinId() == slot6.id then
		...
		slot10 = slot0.shipVO:proposeSkinOwned(slot6) or table.contains(slot0.skinList, slot6.id) or slot9 or slot6.skin_type == ShipSkin.SKIN_TYPE_OLD

		slot8:updateData(slot0.shipVO, slot6, slot10)
		slot8:updateSkin(slot6, slot10)
		slot8.updateUsing(slot8, slot0.shipVO.skinId == slot6.id)
		...
		slot12 = (slot0.shipVO:proposeSkinOwned(slot6) or table.contains(slot0.skinList, slot6.id) or slot11) and 1 or 0

		if slot6.shop_id > 0 and pg.shop_template[slot6.shop_id] or nil then
			slot14 = not pg.TimeMgr.GetInstance():inTime(slot13.time)
		end

		slot15 = slot6.id == slot0.shipVO.skinId
		slot16 = slot6.id == slot0.shipVO:getConfig("skin_id") or slot12 >= 1 or slot6.skin_type == ShipSkin.SKIN_TYPE_OLD
		...
		onButton(slot0, slot7, function ()
			...
			elseif uv2 then
				...
				slot0.emit(slot0, SwichSkinMediator.CHANGE_SKIN, uv1.shipVO.id, uv3.id == uv1.shipVO:getConfig("skin_id") and 0 or uv3.id)
				uv1:back()
			elseif uv4 then
				if uv5 then
