...
function slot0.init(slot0)
	...
	for slot6, slot7 in ipairs(slot0:getRankMsgList()) do
		...
		onToggle(slot0, slot0.rtBtn[slot7], function (slot0)
			if slot0 and not uv0.pageInit[uv1] then
				uv0:emit(BillboardMediator.INIT_RANK, uv0:getRankMsgInfo(uv1))

				uv0.pageInit[uv1] = true
			end
...
function slot0.updateRankTF(slot0, slot1, slot2, slot3)
	...
	if slot2.icon and slot2.icon ~= 0 then
		...
		slot7.id = slot2.icon
		slot7.skinId = slot2.skinId
		slot7.remoulded = slot2.remoulded
		slot7.propose = slot2.proposeTime
...
function slot0.updatePlayerMilitaryRankInfo(slot0)
	...
	slot5.rank = slot0.seasonInfo.rank
	slot5.name = slot0.player.name
	slot5.score = slot0.seasonInfo.score
	slot5.icon = slot0.playerShip.configId
	slot5.skinId = slot0.playerShip.skinId
	slot5.level = slot0.player.level
	slot5.remoulded = slot0.playerShip:isRemoulded()
	slot5.proposeTime = slot0.playerShip.proposeTime
...
function slot0.updatePowerRankTF(slot0, slot1, slot2)
	...
	if slot2.icon and slot2.icon ~= 0 then
		slot5.type = DROP_TYPE_SHIP
		slot5.id = slot2.icon
		slot5.skinId = slot2.skinId
		slot5.remoulded = slot2.remoulded
		slot5.propose = slot2.proposeTime
...
function slot0.updatePlayerRankInfo(slot0, slot1)
	setActive(slot0:findTF("content/player_info", slot0.rtPage[slot1]), true)
	...
	slot7.rank = slot0.myRank[slot1] or 0
	...
	slot7.max = slot3.type == 2 and slot0.collectCountAll or nil
	slot7.isTypePower = slot3.type == 1

	slot0.updatePowerRankTF(slot0, slot2, {})
end
