...
function slot0.register(slot0)
	...
	slot0:bind(uv0.OPEN_SHIP_INFO, function (slot0, slot1)
		if getProxy(BayProxy):getShipById(slot1) then
			slot8.shipId = slot1
			slot8.shipVOs = uv0.contextData.shamChapter:getShips()
...
function slot0.listNotificationInterests(slot0)
	...
	slot1[2] = ChapterProxy.SHAM_CHAPTER_UPDATED
	slot1[3] = GAME.SHAM_ENTER_DONE
	slot1[4] = GAME.SHAM_OP_DONE
	slot1[5] = GAME.SHAM_ENTER_ERROR
	...
	elseif not slot1.fleet:isValid() then
		...
		if slot1.active then
			slot0:save(function ()
				...
				if slot1:getChapterCell(slot0.line.row, slot0.line.column) and slot1.attachment == ChapterConst.AttachRival and slot1.flag == 0 then
					slot6.system = SYSTEM_SHAM
	...
	slot6 = coroutine.create(function ()
		...
		if uv0 == 0 then
			...
			if #slot9 > 0 then
				...
				slot13.type = Player.ASSISTS_TYPE_SHAM
				slot13.callback = uv5
