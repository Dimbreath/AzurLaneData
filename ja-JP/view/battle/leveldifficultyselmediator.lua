...
function slot0.register(slot0)
	...
	slot0:bind(uv0.GO_LEVEL_SENCE, function (slot0, slot1)
		slot2 = uv0.activityProxy

		if not slot2:getActivityById(uv0.contextData.activityId) or slot2:isEnd() then
			pg.TipsMgr.GetInstance():ShowTips(i18n("该活动已结束"))

			return
		end

		slot6.activity_id = slot2.id
		slot6.arg1 = slot1
...
function slot0.removeContext(slot0)
	...
	if slot1:getContextByMediator(ActivityMediator) or slot1:getContextByMediator(MainUIMediator) then
