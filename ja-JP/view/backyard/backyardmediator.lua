...
function slot0.register(slot0)
	...
	slot0:bind(uv0.GO_GRANARY, function (slot0)
		...
		slot1 = uv0
		slot4.mediator = BackyardGranaryMediator
		slot4.viewComponent = BackyardGranaryLayer
	...
	slot0:bind(uv0.GO_SHOP, function (slot0)
		BackYardMediator.isInitAddExpPanel = true
		slot1 = uv0
		slot4.mediator = BackYardShopMediator
		slot4.viewComponent = BackYardShopLayer
	...
	slot0:bind(uv0.GO_SHIPINFO, function (slot0, slot1)
		...
		slot2 = uv0
		slot5.mediator = BackYardShipInfoMediator
		slot5.viewComponent = BackYardShipInfoLayer
		slot6.type = slot1
		slot5.data = {}
...
function slot0.startUpBackyard(slot0, slot1)
	...
	slot7 = pg.backyard
	slot10.ships = slot3
	slot10.furnitures = slot5:getFurnitrues(slot1)
	slot10.level = slot5.level
...
function slot0.listNotificationInterests(slot0)
	slot1[1] = DormProxy.DORM_UPDATEED
	slot1[2] = DormProxy.SHIPS_EXP_ADDED
	slot1[3] = PlayerProxy.UPDATED
	slot1[4] = GAME.PUT_FURNITURE_DONE
	slot1[5] = GAME.OPEN_BACKYARD_GARNARY
	slot1[6] = GAME.BACKYARD_RENAME_DONE
	slot1[7] = BackYardMediator.OPEN_NOFOOD
	slot1[8] = GAME.OPEN_BACKYARD_SHOP
	slot1[9] = BagProxy.ITEM_UPDATED
	slot1[10] = BagProxy.ITEM_ADDED
	slot1[11] = GAME.ADD_SHIP_DONE
	slot1[12] = GAME.EXIT_SHIP_DONE
...
function slot0.handleNotification(slot0, slot1)
	...
	elseif slot2 == DormProxy.SHIPS_EXP_ADDED then
		if not BackYardMediator.isInitAddExpPanel then
			...
			if slot5 ~= 0 and (slot7 ~= 0 or slot6.food ~= 0) then
				...
				slot11.mediator = BackYardSettlementMediator
				slot11.viewComponent = BackYardSettlementLayer
				slot12.oldShips = slot3.oldShips
				slot12.newShips = slot3.newShips
				slot11.data = {}
	...
	elseif slot2 == GAME.OPEN_BACKYARD_GARNARY then
		slot7.mediator = BackYardGarnitureMediator
		slot7.viewComponent = BackYardGarnitureLayer

		slot0:addSubLayers(Context.New({}), nil, function ()
			if uv0 and uv0.callback then
				uv0.callback()
			end
		end)
	elseif slot2 == GAME.OPEN_BACKYARD_SHOP then
		slot7.mediator = BackYardShopMediator
		slot7.viewComponent = BackYardShopLayer
	...
	elseif slot2 == GAME.ADD_SHIP_DONE then
		...
		slot4 = pg.backyard
		slot7.name = BACKYARD.SHIP_ADDED
		slot7.id = slot3.id
	...
	elseif slot2 == GAME.EXIT_SHIP_DONE then
		...
		slot5 = pg.backyard
		slot8.name = BACKYARD.SHIP_EXITED
		slot8.id = slot3.id
