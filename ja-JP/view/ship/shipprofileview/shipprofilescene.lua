...
function slot0.init(slot0)
	...
	slot1[1] = slot0:findTF("bottom/detail")
	...
	slot3[1] = ShipProfileDetailPage.New(slot0.pageTFs, slot0.event)
	slot3[2] = slot1
	slot0.pages = {}
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.obtainBtn, function ()
		slot0.type = MSGBOX_TYPE_OBTAIN
		slot0.shipId = uv0.shipGroup:getShipConfigId()
		slot0.list = uv0.shipGroup.groupConfig.description
		slot0.mediatorName = ShipProfileMediator.__cname
...
function slot0.SwitchPage(slot0, slot1)
	if slot0.index ~= slot1 then
		slot3[1] = function (slot0)
			slot4.groupName = LayerWeightConst.GROUP_SHIP_PROFILE
...
function slot0.CreateLive2D(slot0)
	...
	slot5.configId = slot0.shipGroup:getShipConfigId()
	slot5.skin_id = slot0.skin.id
	slot3.ship = Ship.New({})
	slot3.scale = Vector3(52, 52, 52)
	slot3.position = Vector3(0, -40, 100)
	slot3.parent = slot0.l2dRoot
	slot0.l2dChar = Live2D.New(Live2D.live2dData({}))

	if isHalfBodyLive2D(slot0.skin.prefab) then
		slot5.y = -37 - (slot0.painting.rect.height - slot0.l2dRoot.rect.height * 1.5) / 2
