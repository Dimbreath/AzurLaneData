...
function slot0.playStory(slot0)
	if slot0.miniGameData:GetConfigCsvLine(slot0.linkGameID).story == "" then
		...
		if not slot4:IsPlayed(slot2[1]) then
		...
		onButton(uv0, slot1:Find("display/icon"), function ()
			...
			elseif not uv2:LinkLink(uv3, slot2) then
				...
				GetComponent(slot5, "DftAniEvent"):SetEndEvent(function (slot0)
					...
					for slot5 = 0, uv5.MAX_ROW - 1, 1 do
						for slot9 = 0, uv5.MAX_COLUMN - 1, 1 do
...
function slot0.GameEnd(slot0)
	...
	if slot0.bestScoreTable[slot0.linkGameID] == 0 then
		slot6 = slot0.miniGameData
		slot5.hubid = slot6:getConfig("hub_id")
		slot5.cmd = MiniGameOPCommand.CMD_COMPLETE
		slot6[1] = slot0.linkGameID == #pg.activity_event_linkgame.all and 0 or 1
		slot6[2] = slot0.linkGameID
		slot5.args1 = {}
...
function slot0.IterateByOneSnap(slot0, slot1, slot2, slot3, slot4)
	for slot8 = 1, #slot3, 1 do
		for slot13, slot14 in ipairs(slot0:FindDirectLinkPoint(slot2, slot3[slot8], slot4, slot1)) do
...
function slot0.FindDirectLinkPoint(slot0, slot1, slot2, slot3, slot4)
	...
	for slot9 = slot2.row + 1, uv0.MAX_ROW - 1, 1 do
		slot10 = slot9 .. "_" .. slot2.column
	...
	for slot9 = slot2.column + 1, uv0.MAX_COLUMN - 1, 1 do
		slot10 = slot2.row .. "_" .. slot9
...
function slot0.setIconList(slot0)
	...
	print("当前地图ID", tostring(slot0:GetMGData():GetRuntimeData("curLinkGameID")))
