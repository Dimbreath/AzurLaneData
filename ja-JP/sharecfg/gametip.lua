...
■余燼戦について
■「余燼戦」とは、「余燼」艦船に挑戦し、「解析Pt」を含む報酬入手することができる共闘コンテンツです。
「余燼戦」には下記の2つの要素によって構成されます。
・余燼座標解析＝余燼艦船の挑戦権開放
・余燼艦船との戦闘

「余燼戦」システムはセイレーン作戦のストーリーを進めると開放されます。

■余燼戦座標解析
・「セイレーン作戦」では、戦闘などの方法で「余燼座標情報」を入手することが出来ます。
・「余燼座標情報」作戦海域マップ画面の右上に表示され、余燼座標情報所持数が所持可能上限に達成する度に、余燼座標を全て消費して「余燼戦」の「情報解析」で余燼艦船の挑戦権を開放できます。
・「情報解析」は1日2回しか行なえません。また、毎日0時に情報解析可能回数のほかに、余燼座標所持数は一定数を残して初期所持数にリセットされます。

■余燼戦について
・「セイレーン作戦」海域マップ画面の右下にある「余燼戦」アイコンをタップすると、余燼艦船に挑戦できます。・余燼戦では、
　①自分で余燼座標を解析することで開放される戦闘
　②味方の「余燼戦援軍要請」を受け取った戦闘
　　⇨挑戦するには「座標一覧」を確認する必要があります。
・余燼戦では、味方の「セイレーン作戦に参加した艦隊」で挑戦することができます。また、潜水艦とオフニャを使用できません。
・自分で解析した余燼を撃破した場合、次に開放される余燼は強さが上がった（Lvが上昇）状態で出現します。
・一度の戦闘で敵を撃破できなかった場合、戦闘終了時に「援軍要請」を行い、ほかのプレイヤーに戦闘を開放する事ができます。
・余燼戦では上記①と②のいずれでも、敵が撃破された
（HPが0になった）時、戦闘に参加した指揮官全員「解析Pt」を入手することが出来ます。

■注意
・開放された戦闘は24時間挑戦できます。24時間を経過した場合余燼戦の表示が消失し、挑戦できなくなりま
す。
・自分で解放した戦闘に挑戦するには【燃料】を消費し、余燼のHPが0になるまで何回も挑戦できます。
・味方の救援要請を受け取った戦闘に挑戦するには【挑戦ポイント】を消費します。（挑戦ポイントは毎日回復します）
・同じ戦闘は最大30人まで挑戦することができます。人数が上限いっぱいになった場合、表示された余燼座標でも挑戦することが出来なくなります。
・余燼戦では経験値を入手することが出来ず、また士気コンディション/勝利数にも影響しません。
・余燼戦の出現敵艦変更から31日経過すると、指揮官は自身が開放した余燼戦に挑戦する際に、一度の戦闘に一度だけ、味方支援火力により余燼艦船にダメージ（直接ダメージとスリップダメージの2種類）を与えられるようになります。このダメージは出現敵艦の変更より時間が経てば経つほど上昇していきます。]]
			}
		}
	},
	world_close = {
...
利用不可機能：
退役、強化素材として消費、限界突破素材として消費、軍事委託、ケッコン、認識覚醒、秘書艦への設定、大講堂、セイレーン作戦（余燼戦含む）、大艦隊作戦]]
			}
		}
	},
	help_backyard = {
...
大艦隊兵站は司令/副司令によって管理し、増築させることで得られる効果を強化することが出来ます。
			...
			},
			{
				info = [[
■兵站施設を利用するには
・兵站施設を利用/レベルUPするには、大艦隊の「兵站」で当該兵站項目をタップすることで行えます。
・兵站施設をレベルUPするには、貢献Pt/資金を消費することが出来ます。また、レベルUPするには自身の貢献度がそれぞれのレベルに定められた数値に達成する必要があります。
・兵站施設の最大レベルは、大艦隊全体の兵站施設の最大レベルによって決められます。
・大艦隊から所属していない場合、大艦隊の兵站施設による効果は無効化されます。
・大艦隊から離脱し、別の大艦隊に加入した場合、兵站施設レベルは「自分の兵站施設の最高レベル」か
「大艦隊の兵站施設の最高レベル」の高いほうが適用されます。]]
			},
			{
...
・兵站施設を増築するには、大艦隊司令/副司令は「兵站」画面右上のボタンをタップし「開発モード」を表示させ、更に開発対象（増築対象）を指定する必要があります。
・開発対象となる兵站施設は、大艦隊メンバーが「物資拠出」を行うたびに強化進捗が前進し、進捗ゲージが満タンになりますと、兵站施設のレベル上限が上昇します。
]]
			},
			{
				info = [[
...
「物資拠出」を行うと、大艦隊メンバーは「貢献Pt」、大艦隊は「大艦隊資金」「兵站施設増築進捗」をそれぞれ入手/前進出来ます。
]]
			},
			{
				info = "・開発対象は24時間に1度しか変更できません。また、開発対象のデフォルト設定は「大艦隊容量＋」になります。\n(new!)・毎日の進捗ゲージの進捗可能回数は、大艦隊メンバー数上限×3になります。進捗可能回数が上限に達してしまった場合、「物資拠出」を行っても、進捗ゲージが前進することはありません。（個人貢献Ptは獲得できます）"
			}
		}
	},
	guild_office_tip = {
...
・「META艦船は1種類につき1隻しか所有できず、また「META」艦船は退役/強化素材にすることが出来ません。
・同じ「META」艦船のを2隻目入手した場合、限界突破アイテム「結晶」に変換されます。

■解析Ptについて
・「セイレーン作戦」の「余燼戦」では、特定の「META」艦船に対応する「解析Pt」を入手することが出来ます。
...
・強化を行う際、砲撃/雷撃/航空/装填どれか一つの項目を選択して強化することが出来ます。
・強化を行うと、対応のステータスが上昇します。さらに、強化を一定段階進むと、ステータスの追加ボーナスを得ることが出来ます。
...
・「META」艦船のスキルの習得は素材を消費することで、強化は専用任務をこなし習得EXPを貯めることで行うことができます
　（習得EXPは1日入手できる上限が存在します）
・「META」艦船はそれぞれ1種類のスキルを同時に強化できません。
]]
			}
		}
	},
...
5.友好度報酬は4/28（水）まで毎日1段階ずつ解放されます。]]
			...
			{
				info = "6.仮加入状態の【天城ちゃん】はセイレーン作戦（余燼戦含む）に参加できず、またセイレーン作戦では友好度を獲得できません。"
			},
			{
