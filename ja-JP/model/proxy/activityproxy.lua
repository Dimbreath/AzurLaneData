...
function slot0.register(slot0)
	slot0:on(11200, function (slot0)
		...
		if slot1:getActivityByType(ActivityConst.ACTIVITY_TYPE_CHALLENGE) and not slot1:isEnd() then
			uv0:sendNotification(GAME.CHALLENGE2_INFO, {})
		end

		slot2 = uv0

		if slot2:getActivityByType(ActivityConst.ACTIVITY_TYPE_TASK_LIST_MONITOR) and not slot2:isEnd() and slot2.data1 == 0 then
			uv0:monitorTaskList(slot2)
		end

		slot3 = uv0

		if slot3:getActivityByType(ActivityConst.ACTIVITY_TYPE_BOSS_BATTLE_MARK_2) and not slot3:isEnd() then
	...
	for slot7, slot8 in ipairs(slot0:getPanelActivities()) do
		if slot8:isShow() and not slot8.autoActionForbidden then
			...
			elseif slot9 == ActivityConst.ACTIVITY_TYPE_MONTHSIGN then
				...
				if slot10:STimeDescS(slot3, "*t").year ~= slot8.data1 or slot10.month ~= slot8.data2 then
			...
			elseif slot8.id == ActivityConst.SHADOW_PLAY_ID and slot8.clientData1 == 0 then
				slot11 = getProxy(TaskProxy)

				if not slot11:getTaskById(slot8:getConfig("config_data")[1]) then
					slot12 = slot11:getFinishTaskById(slot10)
				end

				if slot12 and not slot12:isReceive() then
					slot1 = slot8
	...
	if Fleet.SUBMARINE_FLEET_ID <= slot2 then
		if not slot6:isFull() then
			function (slot0, slot1)
				for slot7, slot8 in ipairs(uv0:getActivityRecommendShips(TeamType.TeamToTypeList(slot0), uv1.ships, slot1)) do
...
function slot0.GetVoteBookActivty(slot0)
	if not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_1) and not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_3) and not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_4) and not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_5) and not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_6) and not slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_7) then
		slot1 = slot0:getActivityById(ActivityConst.VOTE_ORDER_BOOK_PHASE_8)
	end
...
function slot0.InitActivityBossData(slot0, slot1)
	...
	if not slot2.normal_expedition_drop_num then
		slot5 = {}
	end
	...
	for slot7, slot8 in slot4(slot5) do
		for slot12, slot13 in pairs(slot8[1]) do
			...
			if not slot3[1][slot13] then
				slot15 = 0
			end

			slot3[1][slot13] = math.max(slot14 - slot15, 0)
			slot16 = slot3[2]

			if not slot3[2][slot13] then
				slot17 = 0
			end
...
function slot0.AddInstagramTimer(slot0, slot1)
	...
	if slot3 then
		if slot3 - pg.TimeMgr.GetInstance():GetServerTime() <= 0 then
			function ()
				slot3.activity_id = uv1
				slot3.cmd = ActivityConst.INSTAGRAM_OP_ACTIVE
				slot3.arg1 = uv2
