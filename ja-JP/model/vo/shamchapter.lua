...
function slot0.update(slot0, slot1)
	...
	_.each(slot0:getConfig("grids"), function (slot0)
		if not slot0[3] then
			slot3.row = slot0[1]
			slot3.column = slot0[2]
			slot2.pos = {}
			slot2.item_type = ChapterConst.AttachNone
			uv0({}).walkable = false
		end
	end)
...
function slot0.getShips(slot0, slot1)
	if slot0:getFriendShip() and defaultValue(slot1, false) then
		table.insert(_(_.values(slot0.ships)):chain():map(function (slot0)
			slot1 = uv0

			if slot1:fetchShipVO(slot0.id) then
...
function slot0.updateFleetShips(slot0, slot1)
	for slot6, slot7 in ipairs(slot1) do
		if slot0.friendAssistShip and slot0.friendAssistShip.id == slot7 then
			slot10.id = slot7
			slot10.hp_rant = slot0.friendAssistShip.hpRant
			slot10.strategies = Clone(slot0.friendAssistShip.strategies)
...
function slot0.isOpen(slot0)
	...
	elseif pg.sim_battle_template[slot0.simId] then
		...
		if slot4:STimeDescS(pg.TimeMgr.GetInstance():GetServerTime(), "*t").month == slot0.simId then
