...
function slot0.getItemCount(slot0, slot1)
	if not slot0.itemsVO[slot1] then
		slot3.id = slot1
		slot2 = Item.New({
			count = 0
		})
...
function slot0.initLines(slot0, slot1)
	...
	for slot14, slot15 in pairs(pg.transform_data_template[slot1].condition_id) do
		...
		slot26 = slot23 or math.abs(slot18.y) > 0

		setActive(slot19:Find("arr"), slot26)
...
function slot0.updateFinished(slot0, slot1)
	...
	slot7.id = id2ItemId(1)
	slot7.count = slot3.use_gold * slot2
	...
	if slot3.use_ship > 0 then
		...
		slot8.id = slot0.shipVO.configId
	...
	if nil then
		...
		for slot13, slot14 in pairs(slot0.shipVO:getShipProperties()) do
			if slot8[slot13] then
				slot19.attrName = AttributeType.Type2Name(slot13)
				slot19.value = math.floor(slot14)
				slot19.addition = slot8[slot13]
		...
		for slot14 = 1, 3, 1 do
			if slot8["equipment_proficiency_" .. slot14] then
				slot20.attrName = EquipType.type2Title(slot14, pg.ship_data_template[slot0.shipVO.configId]["equip_" .. slot14][1]) .. i18n("common_proficiency")
				slot20.value = slot0.shipVO:getEquipProficiencyByPos(slot14) * 100
				slot20.addition = slot8["equipment_proficiency_" .. slot14] * 100

				slot0:updateAttrTF_D(cloneTplTo(slot0.attrTplD, slot0.attrContainer), {}, true)
			end
		end
	end
...
function slot0.updateProgress(slot0, slot1)
	...
	if table.getCount(slot3.effect[(slot0.transforms[slot1] and slot0.transforms[slot1].level or 0) + 1] or {}) > 0 then
		slot10[1] = id2ItemId(1)
		slot10[2] = slot3.use_gold
		...
		if slot3.use_ship ~= 0 then
			...
			if slot9 then
				slot14.id = slot0.shipVOs[slot8[1]].configId
				slot14.type = DROP_TYPE_SHIP
	...
	for slot12 = 1, slot0.itemContainer.childCount, 1 do
		...
		if slot12 <= #slot7 then
			slot15 = ""
			slot18.id = slot14[1]
			slot18.type = DROP_TYPE_ITEM
			slot18.count = (slot7[slot12][1] ~= id2ItemId(1) or setColorStr(slot14[2], slot0.playerVO.gold < slot14[2] and COLOR_RED or COLOR_WHITE)) and setColorStr(slot0:getItemCount(slot14[1]), slot0:getItemCount(slot14[1]) < slot14[2] and COLOR_RED or COLOR_WHITE) .. "/" .. slot14[2]
	...
	if nil then
		...
		for slot15, slot16 in pairs(slot0.shipVO:getShipProperties()) do
			if slot6[slot15] then
				slot21.attrName = AttributeType.Type2Name(slot15)
				slot21.value = math.floor(slot16)
				slot21.addition = slot6[slot15]
		...
		for slot16 = 1, 3, 1 do
			if slot6["equipment_proficiency_" .. slot16] then
				slot22.attrName = EquipType.type2Title(slot16, pg.ship_data_template[slot0.shipVO.configId]["equip_" .. slot16][1]) .. i18n("common_proficiency")
				slot22.value = slot0.shipVO:getEquipProficiencyByPos(slot16) * 100
				slot22.addition = slot6["equipment_proficiency_" .. slot16] * 100

				slot0:updateAttrTF(cloneTplTo(slot0.attrTpl, slot0.attrContainer), {}, true)
			end
		end
	end
	...
	onButton(slot0, slot0.confirmBtn, function ()
		...
		if uv2 then
			...
			slot9.content = i18n("ship_remould_warning_" .. uv2, uv0.shipVO:getName())
...
function slot0.showToolTip(slot0, slot1)
	...
	if not slot3 then
		...
		slot5 = {}
		slot8.name = i18n("common_ship_type")
		slot8.from = ShipType.Type2Name(slot0.shipVO:getShipType())
		slot8.to = ShipType.Type2Name(slot4:getShipType())

		table.insert(slot5, {})

		slot8.name = i18n("attribute_armor_type")
		slot8.from = slot0.shipVO:getShipArmorName()
		slot8.to = slot4:getShipArmorName()
		...
		slot6[7] = AttributeType.Expend
		slot6[8] = AttributeType.Dodge
		slot6[9] = AttributeType.AntiSub
