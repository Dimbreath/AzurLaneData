		...
		for slot6, slot7 in ipairs(slot0.fashionSkins) do
			...
			onButton(slot0, slot9, function ()
				...
				elseif uv2.isSwimsuit ~= 1 then
					...
					slot0.loadPainting(slot0, uv2.painting_hx ~= "" and uv2.painting_hx or uv2.painting)
					uv0:loadSkinBg(uv0.shipVO:rarity2bgPrintForGet(), uv0.shipVO:isBluePrintShip())
				end
		...
		elseif uv1 then
			if HXSet.isHx() and uv2.isSwimsuit == 1 then
				...
				for slot3, slot4 in ipairs(uv3.fashionSkins) do
					...
					slot6.updateSelected(slot6, slot4.id == uv3.fashionSkinId)
					slot6.updateUsing(slot6, uv3.shipVO.skinId == slot4.id)
				end
			end

			slot0 = uv3

			slot0.emit(slot0, ShipInfoMediator.CHANGE_SKIN, uv3.shipVO.id, uv2.id == uv3.shipVO:getConfig("skin_id") and 0 or uv2.id)
		elseif uv4 then
			if uv5 then
				...
				if slot0 then
					slot2.shop_id = uv4.id
					slot1 = Goods.New({}, Goods.TYPE_SKIN)

					if slot1:isDisCount() then
						slot2 = slot1:getConfig("resource_num") * (100 - slot1:getConfig("discount")) / 100
					end

					slot6.content = i18n("text_buy_fashion_tip", slot2, HXSet.hxLan(uv2.name))
