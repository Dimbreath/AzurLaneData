...
function slot0.preload(slot0, slot1)
	...
	slot5[2] = function (slot0)
		...
		if not slot1:HasCacheUI("ShipDetailView") then
...
function slot0.checkToggleActive(slot0, slot1)
	...
	elseif slot1 == ShipViewConst.PAGE.INTENSIFY then
		if not slot0.shipVO:isTestShip() then
			slot2 = not slot0.shipVO:isBluePrintShip()
		else
			slot2 = false
	...
	elseif slot1 == ShipViewConst.PAGE.REMOULD then
		if not slot0.shipVO:isTestShip() then
			return not slot0.shipVO:isBluePrintShip() and pg.ship_data_trans[slot0.shipVO.groupId]
		end
...
function slot0.initShip(slot0)
	...
	slot1[1] = slot0:findTF("painting", slot0.shipInfo)
	slot1[MULTRES] = slot0:findTF("painting2", slot0.shipInfo)
	slot0.tablePainting = {}
...
function slot0.openHelpPage(slot0, slot1)
	if slot1 == ShipViewConst.PAGE.EQUIPMENT then
		...
		slot4.helps = pg.gametip.help_shipinfo_equip.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	elseif slot1 == ShipViewConst.PAGE.DETAIL then
		slot4.type = MSGBOX_TYPE_HELP
		slot4.helps = pg.gametip.help_shipinfo_detail.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	elseif slot1 == ShipViewConst.PAGE.INTENSIFY then
		slot4.type = MSGBOX_TYPE_HELP
		slot4.helps = pg.gametip.help_shipinfo_intensify.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	elseif slot1 == ShipViewConst.PAGE.UPGRADE then
		slot4.type = MSGBOX_TYPE_HELP
		slot4.helps = pg.gametip.help_shipinfo_upgrate.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	elseif slot1 == ShipViewConst.PAGE.FASHION then
		slot4.type = MSGBOX_TYPE_HELP
		slot4.helps = pg.gametip.help_shipinfo_fashion.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	else
		slot4.type = MSGBOX_TYPE_HELP
		slot4.helps = pg.gametip.help_shipinfo_maxlevel.tip
		slot4.weight = LayerWeightConst.THIRD_LAYER
...
function slot0.updatePreference(slot0, slot1)
	...
	for slot11 = 1, slot1:getMaxStar(), 1 do
		slot12 = cloneTplTo(slot0.shipInfoStarTpl, slot5, "star_" .. slot11)

		setActive(slot12:Find("star_tpl"), slot11 <= slot1:getStar())
...
function slot0.displayShipWord(slot0, slot1, slot2)
	...
	if slot2 or not slot0.chatFlag then
		...
		if findTF(slot0.nowPainting, "fitter").childCount > 0 then
			Ship.SetExpression(findTF(slot0.nowPainting, "fitter"):GetChild(0), slot0.paintingCode, slot1, slot0.shipVO:getIntimacy() / 100 + (slot0.shipVO.propose and 1000 or 0))
		end
...
function slot0.switchToPage(slot0, slot1, slot2)
	function slot3(slot0, slot1)
		if slot0 == ShipViewConst.PAGE.DETAIL then
			if slot1 then
				slot2[1] = uv0.detailContainer.rect.width + 200
		...
		elseif slot0 == ShipViewConst.PAGE.EQUIPMENT then
			if slot1 then
				slot2[1] = -(uv0.equipLCon.rect.width + 190)
			...
			if slot1 then
				slot3[1] = uv0.equipRCon.rect.width
		...
		elseif slot0 == ShipViewConst.PAGE.FASHION then
			if slot1 then
				slot2[1] = uv0.fashionContainer.rect.width + 150
	...
	if slot0.viewList[slot1] ~= nil then
		slot4 = slot0.viewList[slot1]

		if not slot4:GetLoaded() then
			slot4:Load()
