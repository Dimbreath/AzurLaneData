...
function slot0.setItems(slot0, slot1)
	if not slot1[ITEM_ID_EQUIP_QUICK_FINISH] then
		slot2 = {
			count = 0,
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.allLaunch, function ()
		...
		if slot0:getNeedCount() > 0 and not uv0.isStopSpeedUpRemind then
			slot1 = pg.MsgboxMgr.GetInstance()
			slot4.content = i18n("ship_buildShipScene_quest_quickFinish", slot0, uv0.itemVO.count == 0 and COLOR_RED or COLOR_GREEN, uv0.itemVO.count)
			slot4.stopRamindContent = i18n("common_dont_remind_dur_login")
	...
	onButton(slot0, slot0.quickCount, function ()
		slot3.id = pg.shop_template[61009].effect_args[1]

		shoppingBatch(61009, {}, 9, "build_ship_quickly_buy_tool")
	end)
end
	...
	if slot2.state == BuildShip.ACTIVE then
		...
		onButton(slot0, slot0:findTF("quick_btn", slot4), function ()
			...
			if uv1.isStopSpeedUpRemind then
				...
				slot6.content = i18n("ship_buildShipScene_quest_quickFinish", 1, uv1.itemVO.count == 0 and COLOR_RED or COLOR_GREEN, uv1.itemVO.count)
...
function slot0.playGetShipAnimate(slot0, slot1, slot2)
	...
	if slot0.findTF(slot0, (pg.ship_data_create_material[slot2].build_anim or "Building") .. "(Clone)") then
	...
	if voiceContent then
		if slot0.loadedCVBankName then
			function ()
				...
				uv0._currentVoice = playSoundEffect(voiceContent)
			end()
		else
