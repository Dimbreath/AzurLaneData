...
function slot0.didEnter(slot0)
	onButton(slot0, slot0:findTF("ok_btn", slot0.mainPanel), function ()
		if uv0.shipVO:isActivityNpc() then
			slot3.content = i18n("npc_strength_tip")
			...
			function slot3.onYes()
				...
				if table.getCount(uv0.contextData.materialShipIds or {}) == 0 then
					...
					if not _.all(slot2:hasEliteShips(uv0.contextData.materialShipIds, uv0.shipVOs), function (slot0)
						...
						slot6 = slot2[1] == "" and "" or ","
						slot4 = pg.MsgboxMgr.GetInstance()
						slot6.content = i18n("destroy_eliteship_tip", string.gsub(slot4, "$1", slot6))
...
function slot0.blurPanel(slot0, slot1)
	if slot1 then
		...
		slot6.pbList = {}
		slot6.groupName = slot0:getGroupNameFromData()
...
function slot0.startModShip(slot0)
	if not slot0.hasAddition then
		slot3.content = i18n("ship_mod_no_addition_tip")
...
function slot0.updateAttr(slot0, slot1)
	...
	setText(slot0:findTF("info_container/addition", slot3), "+" .. (isnan(math.max(math.min(math.floor((slot0.getRemainExp(slot0.shipVO, slot5) + slot9) / slot11), slot12:getModAttrBaseMax(slot5) - slot7[slot5]), 0)) and 0 or slot14))
