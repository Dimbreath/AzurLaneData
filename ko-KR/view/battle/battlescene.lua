...
function slot0.getBGM(slot0)
	return ys.Battle.BattleDataFunction.GetDungeonTmpDataByID(pg.expedition_data_template[slot0.contextData.stageId].dungeon_id).bgm or uv0.super.getBGM(slot0)
end
...
function slot0.init(slot0)
	...
	slot5[2] = {}
	slot5[3] = {}
	slot0._sideSkillFloatStateList[ys.Battle.BattleConfig.FRIENDLY_CODE] = {}
	slot5[1] = {}
	slot5[2] = {}
	slot5[3] = {}
	slot0._sideSkillFloatStateList[ys.Battle.BattleConfig.FOE_CODE] = {}
...
function slot0.appendSkill(slot0, slot1, slot2, slot3, slot4)
	if not slot0._skillFloat then
		slot7.skillName = slot1
		slot7.caster = slot2
		slot7.commander = slot3
		slot7.hrzIcon = slot4
	...
	if slot3 then
		...
		slot15 = ys.Battle.BattleCameraUtil.GetInstance()
		slot17 = slot2:GetMainUnitIndex()

		if slot15:GetCharacterArrowBarPosition(uv1.CameraPosToUICamera(slot15:Clone())) == nil or slot15 == nil and table.contains(TeamType.SubShipType, slot2:GetTemplate().type) and not slot2:IsMainFleetUnit() then
			if slot12 == ys.Battle.BattleConfig.FRIENDLY_CODE then
				slot14 = uv1.CameraPosToUICamera(slot2:GetPosition():Clone():Add(uv0.IN_VIEW_FRIEND_SKILL_OFFSET))
			else
				slot14 = uv1.CameraPosToUICamera(slot2:GetPosition():Clone():Add(uv0.IN_VIEW_FOE_SKILL_OFFSET))
			end
...
function slot0.painting(slot0, slot1, slot2, slot3)
	...
	if slot0._skillPaintings[slot1.id] == nil then
		slot4 = nil
		slot5 = ys.Battle.BattleResourceManager:GetInstance()
		slot5 = slot5.InstPainting(slot5, (HXSet.isHx() or slot1.painting) and (slot1.painting_hx ~= "" and slot1.painting_hx or slot1.painting) or slot1.prefab)
...
function slot0.didEnter(slot0)
	...
	onToggle(slot0, slot0:findTF("AutoBtn"), function (slot0)
		...
		slot4.toggle = uv0:findTF("AutoBtn")
...
function slot0.activeBotHelp(slot0, slot1)
	...
	slot5.helps = i18n("help_battle_auto")
	slot7.sound = SFX_CANCEL
	slot6[1] = {
		text = "text_confirm"
	}
	slot5.custom = {}
...
function slot0.initPauseWindow(slot0)
	...
	onButton(slot0, slot0:findTF("help", slot0.pauseWindow), function ()
		if BATTLE_DEBUG and PLATFORM == 7 then
			...
			slot2.helps = i18n("help_battle_rule")
