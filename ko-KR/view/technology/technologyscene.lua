...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		slot2.helps = pg.gametip.technology_help_text.tip
	...
	onButton(slot0, slot0.refreshBtn, function ()
		if _.any(uv0.technologyVOs, function (slot0)
			...
			slot2.content = i18n("technology_canot_refresh")
		...
		slot2.content = i18n("technology_refresh_tip")
			...
			if slot0 == UIItemList.EventUpdate then
				...
				uv0.lastButtonListener[slot2] = function ()
					...
					if uv0.technologyVOs[uv1 + 1]:getState() == Technology.STATE_FINISHED then
						slot3.id = uv0.technologyVOs[uv1 + 1].id
						slot3.pool_id = uv0.technologyVOs[uv1 + 1].poolId
...
function slot0.onSelected(slot0, slot1)
	...
	function slot5(slot0, slot1)
		...
		for slot8, slot9 in ipairs(uv0) do
			slot2[slot8] = uv0[slot8].localPosition.x - uv0[slot0].localPosition.x
		end
...
function slot0.updateExtraInfo(slot0, slot1)
	...
	if slot1:hasCondition() then
		if not getProxy(TaskProxy):getTaskById(slot1:getTaskId()) then
			slot8.id = slot5
			slot6 = Task.New({})
		end
...
function slot0.updateTechnologyTF(slot0, slot1, slot2, slot3)
	...
	if slot4 == Technology.STATE_IDLE then
		onButton(slot0, slot6, function ()
			...
			if #slot0:getConfig("consume") > 0 then
				slot4.content = i18n("technology_task_build_tip", getDropInfo(slot0))

				function slot4.onYes()
					slot3.id = uv1.id
					slot3.pool_id = uv1.poolId
	...
	elseif slot4 == Technology.STATE_STARTING then
		onButton(slot0, slot7, function ()
			slot2.content = i18n("technology_stop_tip")
...
function slot0.updateItem(slot0, slot1, slot2, slot3)
	...
	onButton(slot0, slot1, function ()
		if #(uv0:getConfig("display_icon") or {}) > 0 then
			slot1.type = MSGBOX_TYPE_ITEM_BOX
			slot1.items = _.map(slot0, function (slot0)
				slot1.type = slot0[1]
				slot1.id = slot0[2]

				return {}
			end)
			slot1.content = uv0:getConfig("display")

			function slot1.itemFunc(slot0)
