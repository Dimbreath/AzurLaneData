...
function slot0.GameBegin(slot0)
	...
	while #slot1 > 0 do
		...
		slot7 = slot0.cards[math.floor(slot2 / (uv0.MAX_COLUMN - 2)) + 1] or {}
		slot0.cards[slot4] = slot7
		slot7.row = slot4
		slot7.column = slot5
		slot7.id = slot1[slot3]
		slot7.state = uv0.CARD_STATE_NORMAL
	...
	for slot6 = 0, uv0.MAX_ROW - 1, 1 do
		for slot10 = 0, uv0.MAX_COLUMN - 1, 1 do
			...
			slot11 = slot0.cards[slot6]

			if not slot0.cards[slot6][slot10] then
				slot12 = {
					row = slot6,
					column = slot10,
...
function slot0.GameLoop(slot0)
	function slot1(slot0)
		...
		for slot6 = 1, #slot0 - 1, 1 do
			...
			slot11 = uv0.bottom:GetChild(slot7.row * uv1.MAX_COLUMN + slot7.column)

			for slot15 = 0, slot11:Find("lines").childCount - 1, 1 do
	...
	function slot2(slot0)
		for slot4 = 1, #slot0 - 1, 1 do
			slot5 = slot0[slot4]
			slot7 = uv1.bottom:GetChild(slot5.row * uv0.MAX_COLUMN + slot5.column)

			for slot11 = 0, slot7:Find("lines").childCount - 1, 1 do
	...
	slot0.list:each(function (slot0, slot1)
		onButton(uv0, slot1:Find("display/icon"), function ()
			...
			elseif not uv2:LinkLink(uv3, slot2) then
				...
				LeanTween.value(go(uv5), 1, 0.15, 0.3):setEase(LeanTweenType.easeInBack):setOnUpdate(System.Action_float(function (slot0)
...
function slot0.GameEnd(slot0)
	...
	if slot0.activityRestTimes > 0 or slot0.lastRecord < slot0.activityBestRecord then
		slot5.activity_id = slot0.activity.id
		slot5.arg1 = slot0.activityProgress + (slot0.activityRestTimes > 0 and 1 or 0)
		slot5.arg2 = slot0.lastRecord
...
function slot0.DisplayResult(slot0, slot1)
	...
	slot2 = slot0.result:Find("bg")

	setActive(slot2:Find("pic_new_record"), slot1.data4 < slot0.activityBestRecord)
...
function slot0.LinkLink(slot0, slot1, slot2)
	...
	slot4.row = slot2.row
	slot4.column = slot2.column

	table.insert({}, slot3)
	table.insert({}, slot3)

	for slot10 = 1, 3, 1 do
		if slot0:IterateByOneSnap({}, slot1.id, slot5, slot6) then
			slot12 = {
				slot11
			}
				...
				while slot11 and slot11.from do
					...
					elseif slot11.from.column ~= slot11.column then
						...
						for slot17 = slot11.column + slot13, slot11.from.column, slot13 do
							slot20.row = slot11.row
							slot20.column = slot17
...
function slot0.IterateByOneSnap(slot0, slot1, slot2, slot3, slot4)
	for slot8 = 1, #slot3, 1 do
		for slot13, slot14 in ipairs(slot0:FindDirectLinkPoint(slot2, slot3[slot8], slot4)) do
...
function slot0.LinkLink1(slot0, slot1, slot2)
	...
	slot4 = {}
	slot6.row = slot1.row
	slot6.column = slot1.column
	slot6.path = {}
	slot4[slot1.row .. "_" .. slot1.column] = {
		rdir = 0,
		cdir = 0,
		snap = 0
	}
	slot5.row = slot1.row
	slot5.column = slot1.column
	slot6 = {
		row = slot2.row,
		column = slot2.column
	}
		...
		_.each({}, function (slot0)
			...
			if not (_.any(uv1, function (slot0)
				...
				if slot4 <= 2 then
					...
					slot10.row = slot0.row
					slot10.column = slot0.column
					slot10.snap = slot4
					slot10.rdir = slot5
					slot10.cdir = slot6
					slot10.path = slot7
