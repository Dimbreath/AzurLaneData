...
function slot0.updateBottomTaskGroup(slot0, slot1)
	...
	triggerToggle(slot2, _.all(slot0.configData[slot1], function (slot0)
		...
		return slot1:getTaskById(slot0) and slot1:isReceive()
	end))
end
...
function slot0.updateTaskGroupDesc(slot0, slot1)
	slot3 = slot0:findTF("main/desc", slot0.taskGroupDesc)
	slot5 = nil
	slot3:Find("Image"):GetComponent(typeof(Image)).sprite = (not slot0.titles[slot1] or slot0.titles[slot1]) and GetSpriteFromAtlas("ui/anniversaryui_atlas", "title" .. slot1)
	slot6 = slot0:findTF("main/task_list", slot0.taskGroupDesc)

	setText(slot3, i18n("anniversary_task_title_" .. slot1))

	function slot8(slot0, slot1)
		if not uv0:getTaskById(slot1) then
			slot4.id = slot1
			slot2 = Task.New({})
		end
