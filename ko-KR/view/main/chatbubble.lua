...
function slot0.update(slot0, slot1)
	...
	slot9.configId = slot4.id

	for slot14 = slot0.stars.childCount, Ship.New({}):getStar() - 1, 1 do
		cloneTplTo(slot0.star, slot0.stars)
	end

	for slot14 = 0, slot0.stars.childCount - 1, 1 do
		slot16 = slot0.stars:GetChild(slot14).gameObject

		slot16.SetActive(slot16, slot14 < slot4.star)
	end
	...
	if slot1.emojiId then
		...
		PoolMgr.GetInstance():GetPrefab("ChatFrame/" .. slot12, slot12, true, function (slot0)
			...
			if uv0.tf and uv0.data then
				...
				for slot7 in string.gmatch(uv1.content, ChatConst.EmojiIconCodeMatch) do
