...
function slot0.confirm(slot0, slot1)
	...
	if slot1.goods:isChargeType() then
		...
		if slot1.goods:isMonthCard() or slot1.goods:isGiftBox() or slot1.goods:isItemBox() then
			...
			-- Decompilation error in this vicinity:
			--- BLOCK #0 158-163, warpins: 1 ---
			if slot1.goods:isGem() then

				-- Decompilation error in this vicinity:
				--- BLOCK #0 164-178, warpins: 1 ---
				slot0:showItemDetail({
					isChargeType = true,
					icon = "chargeicon/" .. slot1.goods:getConfig("picture"),
					name = slot1.goods:getConfig("name"),
					price = numberFormat(slot1.goods:getConfig("money"), ","),
					tagType = slot4,
					normalTip = i18n("charge_start_tip", numberFormat(slot1.goods:getConfig("money"), ","), (slot3 and slot1.goods:getConfig("gem") + slot1.goods:getConfig("gem")) or slot1.goods.getConfig("gem") + slot1.goods:getConfig("extra_gem")),
					onYes = function ()
		...
		if type(pg.item_data_statistics[slot1.goods:getConfig("effect_args")[1]].display_icon) == "table" then
			...
			-- Decompilation error in this vicinity:
			--- BLOCK #1 249-261, warpins: 0 ---
			for slot9, slot10 in ipairs(slot5) do

				-- Decompilation error in this vicinity:
				--- BLOCK #0 249-259, warpins: 1 ---
				table.insert(slot2, {
					type = slot10[1],
					id = slot10[2],
					count = slot10[3]
