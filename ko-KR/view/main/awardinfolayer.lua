...
function slot0.init(slot0)
	...
	slot1.items = slot0._itemsWindow:Find("items")
	slot1.items_scroll = slot0._itemsWindow:Find("items_scroll/viewport/content")
	slot1.ships = slot0._itemsWindow:Find("ships")

	if slot0.title == uv0.TITLE.SHIP then
		slot0.container = ({})["ships"]
...
function slot0.didEnter(slot0)
	slot6.noShip = not slot0.hasShip
...
function slot0.displayAwards(slot0)
	...
	if slot0.title ~= uv0.TITLE.SHIP then
		...
		for slot4 = 1, #slot0.awards, 1 do
			...
			uv1(slot5:Find("ship_tpl"), slot0.awards[slot4])
