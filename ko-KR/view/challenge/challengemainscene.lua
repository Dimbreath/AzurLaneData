...
function slot0.addListener(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		slot2.helps = pg.gametip.challenge_help.tip
	...
	onButton(slot0, slot0.startBtn, function ()
		...
		if not slot0:getActivityByType(ActivityConst.ACTIVITY_TYPE_CHALLENGE) or slot0:isEnd() then
		...
		if uv0:isCrossedSeason() == true then
			...
			if not uv0.curModeInfo then
				slot4.content = i18n("challenge_season_update")
				...
				if slot1 == ChallengeProxy.MODE_CASUAL then
					if not uv0.curModeInfo:getScore() then
						slot3 = uv0.curModeInfo:getLevel()
					end
				end

				slot6.content = i18n(slot2, slot3)
	...
	onButton(slot0, slot0.resetBtn, function ()
		slot2.content = i18n("challenge_normal_reset")
...
function slot0.updateData(slot0)
	...
	if slot0.curMode == ChallengeProxy.MODE_INFINITE then
		...
		if slot0.curModeInfo then
			slot1 = slot0.curModeInfo:getNextInfiniteDungeonIDList()
		else
			slot2 = slot0.challengeInfo
			slot3 = slot0.challengeInfo
			slot1 = (not pg.activity_event_challenge[slot3:getActivityIndex()].infinite_stage[slot2:getSeasonID()][2] or pg.activity_event_challenge[slot3].infinite_stage[slot2][2]) and pg.activity_event_challenge[slot3].infinite_stage[slot2][1]
		end
...
function slot0.updatePaintingList(slot0, slot1, slot2)
	...
	setPaintingPrefabAsync(slot0.painting, slot3[slot4], "chuanwu", function ()
		slot0 = uv0:findTF("fitter", uv0.painting)
	...
	setPaintingPrefabAsync(slot0.paintingShadow1, slot3[slot4], "chuanwu", function ()
		slot0 = uv0:findTF("fitter", uv0.paintingShadow1)
	...
	setPaintingPrefabAsync(slot0.paintingShadow2, slot3[slot4], "chuanwu", function ()
		slot0 = uv0:findTF("fitter", uv0.paintingShadow2)
...
function slot0.updateTimePanel(slot0)
	...
	slot13, slot15, slot16, slot17 = slot5:parseTimeFrom(slot5:GetNextWeekTime(1, 0, 0, 0) - slot5:GetServerTime())
	...
	slot0.timer = Timer.New(function ()
		...
		slot6, slot8, slot9, slot10 = slot0:parseTimeFrom(uv0)
...
function slot0.updateSwitchModBtn(slot0)
	...
	onButton(slot0, slot0.casualModBtn, function ()
		...
		if uv0.curModeInfo then
			if uv0:isCrossedSeason() then
				slot6.content = i18n("challenge_season_update_infinite_switch", uv0.curModeInfo:getLevel() or 0)
		...
		slot4.content = i18n("challenge_infinite_click_switch", slot0)
		slot4.onYes = slot1
	...
	onButton(slot0, slot0.infiniteModBtn, function ()
		...
		if uv0.curModeInfo then
			if not uv0.curModeInfo:getScore() then
				slot0 = uv0.challengeInfo:getGradeList().seasonMaxScore
			end
		end

		if uv0:isCrossedSeason() then
			slot6.content = i18n("challenge_season_update_casual_switch", slot0)
		...
		slot4.content = i18n("challenge_casual_click_switch", slot0)
		slot4.onYes = slot1
...
function slot0.updateAwardPanel(slot0)
	if not getProxy(TaskProxy):getTaskById(pg.activity_template[pg.activity_template[getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_CHALLENGE).id].config_data[1]].config_data[1]) then
		slot4 = getProxy(TaskProxy):getFinishTaskById(slot3)
	end
...
function slot0.showSLResetMsgBox(slot0)
	...
	if slot1 == true then
		...
		slot8.content = i18n(slot4, slot5)
...
function slot0.openCommanderPanel(slot0, slot1, slot2)
	...
	slot0.levelCMDFormationView:ActionInvoke("update", slot1, slot0.commanderPrefabs, function (slot0)
		if slot0.type == LevelUIConst.COMMANDER_OP_ADD then
			slot2.fleetIndex = uv1
			slot2.cmdPos = slot0.pos
			slot2.mode = uv0.curMode
...
function slot0.closeCommanderPanel(slot0)
	if slot0.levelCMDFormationView then
		if slot0.levelCMDFormationView:GetLoaded() then
			if not slot0.levelCMDFormationView:CheckState(BaseSubView.STATES.DESTROY) then
