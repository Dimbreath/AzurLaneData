...
function slot0.UpdateConsumeComparer(slot0)
	slot0.consumePanel:Find("GoldText"):GetComponent(typeof(Text)).text = setColorStr(slot0.consumeGold or 0, (slot0.player.gold or 0) >= (slot0.consumeGold or 0) and COLOR_WHITE or COLOR_RED)
...
function slot0.UpdateFormula(slot0, slot1)
	...
	slot0:UpdateConsume(slot0.contextData.formula.coin_consume)
...
function slot0.RefreshEquipmentInstance(slot0)
end
...
function slot0.CheckEnoughMaterials(slot0)
	...
	if slot0.equipmentSourceId ~= 0 and not slot0.contextData.sourceEquipmentInstance then
		return
	end

	slot2, slot3 = TransformEquipmentCommand.CheckEquipmentFormulasSucceed({
		slot0.contextData.formulaId
	}, slot0.equipmentSourceId ~= 0 and slot0.contextData.sourceEquipmentInstance.id or slot0.equipmentSourceId)
...
function slot0.UpdateSourceInfo(slot0)
	slot2 = slot0.contextData.sourceEquipmentInstance and slot1.id or slot0.equipmentSourceId
	...
	updateDrop(slot0.sourceEquipItem:Find("Item"), {
		id = slot2,
		type = DROP_TYPE_EQUIP
	})
	...
	slot5 = slot0.sourceEquipItem:Find("Status")

	if not slot0.childsCanUse then
		setImageSprite(slot5, LoadSprite("ui/equipmenttransformui_atlas", "noown"))
		setActive(slot5, true)
	elseif not slot1 then
		setImageSprite(slot5, LoadSprite("ui/equipmenttransformui_atlas", "unselect"))
		setActive(slot5, true)
	else
		setActive(slot5, false)
	end

	slot6 = slot1 and slot1.shipId

	setActive(slot0.sourceEquipItem:Find("EquipShip"), slot6)

	if slot6 then
		slot0.loader:GetSprite("qicon/" .. getProxy(BayProxy):getShipById(slot6):getPainting(), "", slot0.sourceEquipItem:Find("EquipShip/Image"))
	end
end
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.consumePanel:Find("ComposeBtn"), function ()
		...
		slot1 = uv0.contextData.sourceEquipmentInstance

		if uv0.equipmentSourceId ~= 0 and slot1.shipId ~= nil then
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				content = i18n("equipment_upgrade_feedback_equipment_consume", getProxy(BayProxy):getShipById(slot1.shipId):getName(), slot1.config.name),
				onYes = function ()
					uv0:emit(EquipmentTransformMediator.TRANSFORM_EQUIP, uv1, uv0.contextData.formulaId)
				end
			})

			return
		else
			uv0:emit(EquipmentTransformMediator.TRANSFORM_EQUIP, slot1, uv0.contextData.formulaId)
		end
	end, SFX_PANEL)
