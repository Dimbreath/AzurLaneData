...
9. 6월 18일 전 매일 완성 가능한 그림이 하나씩 해제됩니다.]]
			}
		}
	},
	link_link_help_tip = {
...
입는 피해 배율, 적군의 피해량 증가]]
	...
	shop_street_activity_tip = {
		tip = "·이벤트 기간(<color=#A1E400FF>2020.09.24-2020.10.7</COLOR>)에는 갱신 시마다\n반드시 <color=#A1E400FF>장비 스킨 상자</COLOR>가 출현합니다."
	},
	twitter_link_title = {
		tip = "连接中的推特账号：$1"
	},
...
수 있습니다.]]
 ...
 수 있습니다.]]
	...
	buildship_new_tip = {
		tip = "건조 횟수 200회 누적 시마다 UR 함선 {namecode:182} 획득 가능(최대 4회)"
	},
	buildship_heavy_tip = {
		tip = "나가토 ,스즈야 건조 추가 "
	},
...
이벤트 규칙:
1.9월 30일까지 매일 0시마다 임무 1개가 해제되며, 이벤트 임무를 완료하여
「축제 건축 자재」, 「탐색 PT」 및 보상을 획득할 수 있다. 이벤트 임무 누적
가능.
2.「축제 건축 자재」를 소모하여 축제 광장에서 건물을 업그레이드할 수 있으며, 각 건물
마다 다른 버프 속성을 제공한다.
3. 매 건물은 3개의 레벨로 나눠져 있으며 건물의 레벨이 상승할 때마다 제공하는
버프 속성도 상승한다.
4. 업그레이드 가능한 건물:
쥬쥬 카페
쥬쥬 피규어
...
이동 시간 내에 구슬을 상하좌우 거리에 상관없이 이동시킬 수 있다.
2. 이동 과정에서 경로 상의 구슬과 위치가 교환되며, 7초가 지나거나
손을 떼면 제거 및 결산이 시작된다.
3. 같은 색 구슬을 가로 또는 세로로 3개 이상 연결하면 제거할 수 있고, 한 번에
제거하는 구슬이 많을수록 고득점.
이벤트 규칙:
이벤트 기간 동안 매일 사용 가능 횟수를 얻으며, 최대 7회까지 얻을 수
 있다.
1. 사용 가능 횟수가 남아있는 상황에서 쥬쥬 롤링볼에 참여하면 횟수를 소모하며
출석 횟수를 얻는다. 출석 7회 시
「축제 초대장」을 획득한다.
2. 사용 가능 횟수는 축제 광장 화면 우측에서 확인할 수 있고, 출석 진도는
축제 광장 화면의 「초대장」 버튼을 통해 확인할 수 있다.
3. 사용 가능 횟수가 없을 경우 쥬쥬 롤링볼에 참여하면 출석 횟수를 얻지
못한다.
4. 기타 원인으로 인해 중도 퇴장할 경우 사용 가능 횟수는 차감되지 않고
어떠한 보상도 얻지 못한다.]]
			}
		}
	},
		...
		tip = {
			{
				info = [[
게임 규칙:
1. 이벤트 기간 동안, 스테이지 내 칸을 터치하여 스테이지를 탐색할 수 있다.
스테이지 탐색 시 탐색 PT를 소모한다.
2. 스테이지 탐색도가 100%에 도달해야 다음 스테이지로 이동할 수 있다.
3. 이미 탐색한 구역과 인접한 칸만 탐색이 가능하며, 탐색 완료 시
탐색도가 증가한다.
4. 스테이지 내 칸은 3가지 유형으로 나뉜다: 보통칸, 상자칸, 적군칸.
각 칸마다 탐색 완료 후의 보상은 다르다.
주의: 상자칸은 상자를 오픈해야 하며, 적군칸은 적을 처치해야 탐색이
완료된다.]]
			}
		}
	},
...
이상 이벤트 내용은 모두 누적 가능
2. 「탐색 PT」를 소모하여 「쥬쥬 모험기」 스테이지 내에서 맵을 탐색하여
풍부한 보상 획득 가능]]
			}
		}
	},
...
스탯 부분
1. 기본 탐지 범위
전열 함대의 구축함과 경순양함의 대잠 수치와 공식에 따라 기본
탐지 직경 계산:
...
형성된다.
 ·후열에 편성된 모든 캐릭터들의 대삼 수치 합계 후, 공식에 따라 추가 범위를
계산한다.
