return {
	fadeOut = 1.5,
	mode = 2,
	fadeType = 2,
	once = true,
	id = "FUXIANGXIANZUOZHAN26",
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			bgmDelay = 2,
			bgm = "battle-executor-type1",
			say = "메탈 블러드 부도 요새·D12 방어 모듈",
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401460,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "조심해, 비상사태 발생...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "페터, 거울 해역이 사라지고 있어. 뭘 한 거야?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "에? 그건 중앙 모듈에서 통제하는 건데. 난 아무것도... 중앙 통제 섬의 거울 해역 생성기에 문제가 생긴 건가?! 하지만 왜 이런 때에...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "...정찰 상황이 변했어. 해역 내에 \"메탈 블러드\"와 \"사쿠라 엠파이어\" 함대가 전부 사라졌어.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 406010,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "그리고 목표 구역에는 프린츠 오이겐과 강력한 세이렌 하나만 있는데.... 아마도 세이렌 지휘 체계인 컴파일러로 추정돼.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "정찰기를 속여 넘길 수 있는 것은 우리의 기술일 거야...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "프린츠 오이겐이 직접 가짜 메탈 블러드 함대와 사쿠라 엠파이어 함대를 만들었군... 그러니까 저 프린츠 오이겐은 진짜지.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "지금까지 봤던 메탈 블러드, 사쿠라 엠파이어, 사르데냐 제국 함대 역시 모두 진짜일 거야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "그렇군... 이제 알겠어. 대단하네, 오이겐. 이런 상황에서 작전을 세우다니...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401460,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "오, 너희는 이해했어? 이게 대체 무슨 일인지...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "오늘 일어났던 짧은 통신 고장이 시작일 거야. 프린츠 오이겐은 오늘 함대를 데리고 올 생각이었겠지. 사쿠라 엠파이어도 마찬가지고.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "하지만 우리는 그런 소식을 듣지 못하고 계획대로 훈련을 진행한 거야. 물론, 시스템에 침입한 세이렌은 모든 계획을 확실하게 알고 있었지.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401460,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "그래서 이런 혼란이 빚어진 건가... 그렇다면 오이겐이 해역에 들어왔을 때 연락해 주면 좋았잖아?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "이미 했지만 우리 통신 시스템이 세이렌에게 당해서 고장난 것처럼 들렸을 거야. 사실은 통제를 받고 있던 거지만.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "그걸 알아차린 프린츠 오이겐이 이 모든 것을 계획했던 거야. 결국 마지막에 세이렌의 진짜 위치를 밝혀낸 거고...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "오이겐, 정말 대단하네... 세이렌의 농간이라고는 생각했지만 이렇게 복잡할 줄은 몰랐어.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "하지만 잠시만... 바깥은 그렇다 쳐도, 우리 통신은 멀쩡했잖아.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "오이겐을 쫓아간 하인리히는 계속 거기에 있었는데... 설마 알고 있었던 건가?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "후후, 그런 것 같네... 우리의 목표 지점도 아마 하인리히가 알려줬을 거야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "신중한 세이렌이 계속 해역에 머문 이유는 우리가 여전히 자기 계산대로 서로 싸우고 있다고 생각했기 때문이겠지.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "만약 이 정보가 통신기를 통해서 새어나갔다면 분명 숨었을 거야. 그렇게 되면 다시 찾기가 번거로웠을 거고.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401460,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "적을 속이려면 아군도 속여야 한다는 건가.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "그렇다면 하인리히가 완벽하게 임무를 수행했다는 거네. 아까 보고에서는 아무런 위화감도 느끼지 못했는데...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 406010,
			nameColor = "#a9f548",
			say = "잠시만, 뭔가 변했어! 컴파일러가 들킨 뒤 싸우는 것이 아니라 요새 밖으로 이동하고 있어. 도망치는 건가...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_3",
			dir = 1,
			actor = 407030,
			nameColor = "#a9f548",
			say = "최고 기밀인 메탈 블러드의 기지에 침입자가 생겼는데... 이렇게 놓치면 체면이 말이 아니겠지?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			nameColor = "#a9f548",
			side = 2,
			dir = 1,
			bgName = "bg_fuxiangxian_4",
			bgm = "bsm-9",
			actorName = "페터 슈트라서",
			say = "해역에 있는 모든 메탈 블러드, 그리고 우리의 동맹 사쿠라 엠파이어와 샤르데나 제국에게 알린다.",
			flashout = {
				dur = 0.25,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.25,
				dur = 0.25,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "페터 슈트라서",
			side = 2,
			bgName = "bg_fuxiangxian_4",
			nameColor = "#a9f548",
			dir = 1,
			say = "나는 메탈 블러드 제2 주력함대의 기함, 이 기지의 책임자 페터 슈트라서.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "페터 슈트라서",
			side = 2,
			bgName = "bg_fuxiangxian_4",
			nameColor = "#a9f548",
			dir = 1,
			say = "우리 요새는 세이렌 컴파일러의 습격을 받았다. 그리고 약 3분 전, 침입을 당했다는 사실을 발견했지.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "페터 슈트라서",
			side = 2,
			bgName = "bg_fuxiangxian_4",
			nameColor = "#a9f548",
			dir = 1,
			say = "적색 증축 동맹은 구성원에 대한 공격 행위를 동맹 모두에 대한 공격으로 간주한다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "페터 슈트라서",
			side = 2,
			bgName = "bg_fuxiangxian_4",
			nameColor = "#a9f548",
			dir = 1,
			say = "함께 손잡고 적에게 맞서 싸우자! ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "페터 슈트라서",
			side = 2,
			bgName = "bg_fuxiangxian_4",
			dir = 1,
			blackBg = true,
			nameColor = "#a9f548",
			say = "꿈도 꾸지 마... 페터 슈트라서, 통신 종료.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
