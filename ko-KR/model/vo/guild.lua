...
slot1[1] = i18n("guild_policy_power")
slot1[MULTRES] = i18n("guild_policy_relax")
slot0.POLICY_NAME = {}
slot1[1] = i18n("guild_faction_blhx")
slot1[MULTRES] = i18n("guild_faction_cszz")
slot0.FACTION_NAME = {}
...
function slot0.updateBaseInfo(slot0, slot1)
	...
	slot0.updateResource(slot0, slot2.resource or 0)
	...
	for slot7, slot8 in ipairs(slot2.buff_list or {}) do
		slot11.id = slot8.buff_id
		slot11.level = slot8.level
...
function slot0.changeFactionLeftTime(slot0)
	...
	return slot2:parseTimeFrom(slot0.changeFactionTime - slot2:GetServerTime())
end
...
function slot0.getEnableDuty(slot0, slot1, slot2)
	...
	if slot1 == GuildMember.DUTY_COMMANDER then
		if slot0:getAssistantMaxCount() == slot0:getAssistantCount() then
			if slot2 == GuildMember.DUTY_DEPUTY_COMMANDER then
				slot4[1] = GuildMember.DUTY_COMMANDER
				slot4[2] = GuildMember.DYTY_PICKED
				slot4[3] = GuildMember.DUTY_ORDINARY

				if not {} then
					slot4[1] = GuildMember.DYTY_PICKED
					slot4[2] = GuildMember.DUTY_ORDINARY
					slot3 = {}
				end
			end
		elseif slot2 == GuildMember.DUTY_DEPUTY_COMMANDER then
			slot4[1] = GuildMember.DUTY_COMMANDER
			slot4[2] = GuildMember.DUTY_DEPUTY_COMMANDER
			slot4[3] = GuildMember.DYTY_PICKED
			slot4[4] = GuildMember.DUTY_ORDINARY

			if not {} then
				slot4[1] = GuildMember.DUTY_DEPUTY_COMMANDER
				slot4[2] = GuildMember.DYTY_PICKED
				slot4[3] = GuildMember.DUTY_ORDINARY
				slot3 = {}
			end
