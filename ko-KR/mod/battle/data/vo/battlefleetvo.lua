...
slot0.Battle.BattleFleetVO = slot7
...
function slot7.init(slot0)
	...
	slot1[1] = slot0._submarineDiveVO
	slot1[2] = slot0._submarineFloatVO
	...
	slot0._fleetSonar = uv0.Battle.BattleFleetSonar.New(slot0)
...
function slot7.FleetWarcry(slot0)
	...
	slot1:DispatchChat(uv0.Battle.BattleDataFunction.GetWords(slot1:GetSkinID(), slot5, slot1:GetDefaultSkinID()), 2.5, slot5)
end
...
function slot7.SubWarcry(slot0)
	...
	slot1:DispatchChat(uv0.Battle.BattleDataFunction.GetWords(slot1:GetSkinID(), slot2, slot1:GetDefaultSkinID()), 2.5, slot2)
end
...
function slot7.fireChargeWeapon(slot0, slot1, slot2, slot3)
	...
	if slot2 then
		if slot0._IFF == uv0.FRIENDLY_CODE then
			slot0._chargeWeaponVO:PlayCutIn(slot1:GetHost(), 1 / uv0.FOCUS_MAP_RATE)
		end
...
function slot7.CoupleEncourage(slot0)
	...
	for slot11, slot12 in ipairs(slot0._unitList) do
		...
		slot1[uv0.GetPlayerShipModelFromID(slot12:GetTemplateID()).group_type] = true
		...
		if #uv0.GetWords(slot12:GetSkinID(), "couple_encourage", slot12:GetDefaultSkinID()) > 0 then
			slot6[slot12] = slot22
		end
	end
	...
	for slot12, slot13 in pairs(slot6) do
		for slot17, slot18 in ipairs(slot13) do
			...
			if slot22 and slot20 <= slot22 then
				slot23.cp = slot12
				slot23.content = slot18[3]
				slot23.linkIndex = slot17
				slot7[#slot7 + 1] = {}
			end
		end
	end
...
function slot7.ChangeSubmarineState(slot0, slot1, slot2)
	...
	if slot2 then
		...
		if slot0._submarineShiftVO:IsOverLoad() and uv0.SR_CONFIG.DIVE_CD < slot0._submarineShiftVO:GetMax() - slot0._submarineShiftVO:GetCurrent() then
			-- Nothing
		else
