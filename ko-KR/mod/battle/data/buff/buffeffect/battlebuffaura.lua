	...
	slot0._aura = uv0.Battle.BattleDataProxy.GetInstance():SpawnLastingCubeArea(uv1.AOEField.SURFACE, slot1:GetIFF(), Vector3(-55, 0, 55), 180, 70, 0, function (slot0)
		for slot4, slot5 in ipairs(slot0) do
			if slot5.Active then
				for slot10, slot11 in ipairs(uv0:getTargetList(uv1, uv0._target, uv0._tempData.arg_list)) do
		...
		if slot0.Active then
			for slot5, slot6 in ipairs(uv0:getTargetList(uv1, uv0._target, uv0._tempData.arg_list)) do
