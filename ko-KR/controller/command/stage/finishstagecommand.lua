...
function slot0.CheaterVertify()
	...
	slot0, slot5.reason = ys.Battle.BattleState.Vertify()
...
function slot0.GeneralPackage(slot0, slot1)
	...
	for slot11, slot12 in ipairs(slot1) do
		if slot0.statistics[slot12.id] then
			...
			slot22.ship_id = slot14
			slot22.hp_rest = slot15
			slot22.damage_cause = slot16
			slot22.damage_caused = math.floor(slot13.damage)
			slot22.max_damage_once = slot18
			slot22.ship_gear_score = math.floor(slot13.gearScore)
	...
	slot10.score = slot6
	slot10.key = slot7
	slot10.statistics = slot3
	slot10.kill_id_list = slot0.statistics.kill_id_list
	slot10.total_time = slot0.statistics._totalTime
	slot10.bot_percentage = slot0.statistics._botPercentage
	slot10.extra_param = slot2
	slot10.file_check = tostring(math.floor(GetBattleCheck() % 88824 * (slot0.token % 88824) % (88824 + slot7)))
	slot10.enemy_info = {}
	slot10.data2 = {}
...
function slot0.GenerateCommanderExp(slot0, slot1)
	...
	for slot9, slot10 in ipairs(slot0.commander_exp) do
		...
		if slot13:isMaxLevel() then
			slot17.commander_id = slot11
			slot17.curExp = slot13.exp
			...
			slot17.commander_id = slot11
			slot17.exp = slot10.exp
			slot17.curExp = slot14
	...
	slot9.surfaceCMD = slot6
	slot9.submarineCMD = slot7
