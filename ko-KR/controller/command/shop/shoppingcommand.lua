...
function slot0.execute(slot0, slot1)
	...
	if slot5.discount ~= 0 then
		if table.getCount(slot5.discount_time) ~= 0 then
			if pg.TimeMgr.GetInstance():inTime(slot5.discount_time) then
	...
	if slot7[id2res(slot5.resource_type)] < slot12 then
		...
		if slot5.resource_type == 1 then
			slot18[2] = slot12 - slot7[id2res(slot5.resource_type)]
			slot18[3] = slot12
			slot17[1] = {
				59001
			}
	...
	pg.ConnectionMgr.GetInstance():Send(16001, {}, 16002, function (slot0)
		if slot0.result == 0 then
			...
			if uv0.type ~= 0 then
				if uv0.is_auto_use == 1 then
					...
					slot4.type = uv0.type
					slot4.id = uv0.effect_args[1]
					slot4.count = slot2 * uv3
					slot3 = Item.New({})
			...
			slot2 = uv2:getData()
			slot5[id2res(uv0.resource_type)] = uv5
			...
			elseif uv0.genre == ShopArgs.SkinShop then
				slot7.id = uv0.effect_args[1]

				getProxy(ShipSkinProxy):addSkin(ShipSkin.New({}))
			elseif uv0.genre == ShopArgs.SkinShopTimeLimit then
				slot5 = getProxy(ShipSkinProxy)

				if slot5:getSkinById(uv0.effect_args[1]) and slot6:isExpireType() then
					slot9.id = slot4
					slot9.end_time = uv0.time_second * uv3 + slot6.endTime

					slot5:addSkin(ShipSkin.New({}))
				elseif not slot6 then
					slot9.id = slot4
					slot9.end_time = uv0.time_second * uv3 + pg.TimeMgr.GetInstance():GetServerTime()
			...
			slot4 = uv1
			slot7.id = uv4
			slot7.shopType = slot3
			slot7.normalList = uv7:GetNormalList()
			slot7.normalGroupList = uv7:GetNormalGroupList()
			slot7.awards = uv0.is_auto_use == 1 and slot1 or {}

			slot4.sendNotification(slot4, GAME.SHOPPING_DONE, {})
		else
			print(slot0.result)
