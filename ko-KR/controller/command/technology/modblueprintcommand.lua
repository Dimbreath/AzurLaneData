...
function slot0.execute(slot0, slot1)
	...
	slot18.ship_id = slot6.shipId
	slot18.count = slot3

	pg.ConnectionMgr.GetInstance():Send(63204, {}, 63205, function (slot0)
		if slot0.result == 0 then
			...
			if Clone(uv3).level < uv3.level then
				for slot6 = slot1.level + 1, uv3.level, 1 do
					...
					if slot7:getStrengthenConfig(slot6).special == 1 and type(slot7.special_effect) == "table" then
						for slot12, slot13 in ipairs(slot7.special_effect) do
							if slot13[1] == ShipBluePrint.STRENGTHEN_TYPE_SKILL then
								for slot21, slot22 in ipairs(slot13[2]) do
									slot25.id = slot22
									getProxy(BayProxy):getShipById(uv3.shipId).skills[uv4] = {
										exp = 0,
										level = 1
									}
			...
			elseif slot1.fateLevel < uv3.fateLevel then
				for slot6 = slot1.fateLevel + 1, uv3.fateLevel, 1 do
					...
					if slot7:getFateStrengthenConfig(slot6).special == 1 and type(slot7.special_effect) == "table" then
						for slot12, slot13 in ipairs(slot7.special_effect) do
							if slot13[1] == ShipBluePrint.STRENGTHEN_TYPE_CHANGE_SKILL then
								...
								if slot20:getStudentByShipId(slot16.id) and slot21.skillId == slot17 then
			...
			slot3.strengthList = {}
			slot6.level = uv3.level + math.max(uv3.fateLevel, 0)
			slot6.exp = uv3.exp
			...
			slot7.oldBluePrint = slot1
			slot7.newBluePrint = uv3
