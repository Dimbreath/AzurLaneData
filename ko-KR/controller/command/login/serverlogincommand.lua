		...
		if pg.ConnectionMgr.GetInstance():getConnection() then
			if pg.ConnectionMgr.GetInstance():isConnected() then
				function (slot0)
					...
					slot4.platform = uv0
					slot4.account_id = uv1.uid
					slot4.server_ticket = slot0 or uv1.token
					slot4.serverid = uv2.id
					slot4.check_key = HashUtil.CalcMD5(uv1.token .. AABBUDUD)
					slot4.device_id = pg.SdkMgr.GetInstance():GetDeviceId()

					slot1.Send(slot1, 10022, {}, 10023, function (slot0)
