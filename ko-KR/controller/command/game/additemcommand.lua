...
function slot0.execute(slot0, slot1)
	if slot1:getBody().dropType == DROP_TYPE_RESOURCE then
		...
		slot5 = slot4:getData()
		slot8[id2res(slot2.id)] = slot2.count
	...
	elseif slot2.dropType == DROP_TYPE_WORLD_ITEM then
		...
		slot9.item = slot2.id
		slot9.itemnum = slot2.count

		slot4:AddLog(WorldLog.TypeDrop, {})
	elseif slot2.dropType == DROP_TYPE_ICON_FRAME then
		slot6.id = slot2.id
		slot5 = IconFrame.New({})
		slot9.end_time = pg.TimeMgr.GetInstance():GetServerTime() + slot5:getConfig("time_second")
