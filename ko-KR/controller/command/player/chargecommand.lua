	...
	pg.ConnectionMgr.GetInstance():Send(11501, {
		...
		if slot0.result == 0 then
			if (PLATFORM_CODE == PLATFORM_JP or PLATFORM_CODE == PLATFORM_US) and pg.SdkMgr.GetInstance():GetIsPlatform() then
				...
				pg.SdkMgr.GetInstance():SdkPay(uv0:getConfig("id_str"), uv0:getConfig("money") * 100, uv0:getConfig("name"), uv0:isMonthCard() and 1 or uv0:firstPayDouble() and uv1 and uv0:getConfig("gem") * 2 or uv0:getConfig("gem") + uv0:getConfig("extra_gem"), slot8, uv0:getConfig("subject"), "-" .. slot3.id .. "-" .. slot8, slot3.name, notify_url, order_sign)
			end
