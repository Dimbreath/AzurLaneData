...
function slot0.set(slot0, slot1, slot2)
	...
	onToggle(slot0, slot0.commanderBtn, function (slot0)
		slot0.parent.contextData.showCommander = slot0
...
function slot0.updateCommanderBtn(slot0, slot1, slot2)
	...
	setActive(slot0:findTF("btn_clear", slot5), slot2 <= slot0:getLimitNums(slot1) and not slot0.parent.contextData.showCommander)
	setActive(slot0:findTF("btn_recom", slot5), slot2 <= slot0.getLimitNums(slot1) and not slot0.parent.contextData.showCommander)
	setActive(slot0:findTF("blank", slot5), not (slot2 <= slot0.getLimitNums(slot1)) or (slot2 <= slot0.getLimitNums(slot1) and not slot0.fleets[slot1][slot2] and slot0.parent.contextData.showCommander))
	setActive(slot0:findTF("commander", slot5), slot0.parent.contextData.showCommander and slot2 <= slot0.getLimitNums(slot1) and slot0.fleets[slot1][slot2])
end
...
function slot0.updateFleet(slot0, slot1, slot2)
	...
	if slot4 then
		...
		onButton(slot0, slot11, function ()
			slot0.parent:emit(ActivityBossBattleMediator2.ON_FLEET_RECOMMEND, slot1.id)
		end)
		onButton(slot0, slot12, function ()
			slot0.parent:emit(ActivityBossBattleMediator2.ON_FLEET_CLEAR, slot1.id)
		end, SFX_UI_CLICK)
	end
end
...
function slot0.updateCommanders(slot0, slot1, slot2)
	for slot6 = 1, 2, 1 do
		...
		onButton(slot0, slot9, function ()
			slot0.parent:openCommanderPanel(slot0.parent, slot1.id)
		end, SFX_PANEL)
...
function slot0.updateShips(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	slot6:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			if getProxy(BayProxy):getShipById(slot0[slot1 + 1]) then
			...
			function slot6()
				slot0.onCancel()
				slot0.onCancel.parent:emit(ActivityBossBattleMediator2.ON_OPEN_DOCK, {
					shipType = 0,
					fleet = slot1,
					shipVO = slot2,
					fleetIndex = slot3,
			...
			slot5.onLongPressed:AddListener(function ()
				if slot0 then
					slot1.onCancel()
					slot1.onLongPressShip(slot0.id, _.map(slot2:getShipIds(), function (slot0)
