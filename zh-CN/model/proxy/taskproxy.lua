...
function slot0.register(slot0)
	slot0:on(20001, function (slot0)
		...
		for slot4, slot5 in ipairs(slot0.info) do
			...
			if slot6:getConfigTable() ~= nil then
...
function slot0.dealMingshiTouchFlag(slot0, slot1)
	...
	if not slot2:getActivityById(mingshiActivityId) or slot2:isEnd() then
		return
	end
...
function slot0.mingshiTouchFlagEnabled(slot0)
	...
	if not slot1:getActivityById(mingshiActivityId) or slot1:isEnd() then
		return
	end
	...
	if Task.New({
		...
		return _.all(slot3, function (slot0)
			if not uv0:getTaskById(slot0) then
				slot1 = uv0:getFinishTaskById(slot0)
			end
...
function slot0.isReceiveTasks(slot0, slot1)
	return _.all(slot1, function (slot0)
		...
		return slot1:getFinishTaskById(slot0) and slot1:isReceive()
	end)
end
