...
function slot0.register(slot0)
	slot0:on(50101, function (slot0)
		...
		elseif slot0.type == ChatConst.CODE_ACTOBSS_MSG_WORD then
			...
			slot5:addNewMsg(ChatMsg.New(ChatConst.ChannelWorld, {
				player = Player.New(slot0.player),
				content = slot2,
				emojiId = slot3,
				timestamp = slot5:GetServerTime()
			}))
		end
	end)
	slot0:on(50103, function (slot0)
		...
		slot3:addNewMsg(ChatMsg.New(ChatConst.ChannelPublic, {
			id = slot0.ad_id,
			args = slot1,
			timestamp = slot3:GetServerTime()
		}))
	end)
