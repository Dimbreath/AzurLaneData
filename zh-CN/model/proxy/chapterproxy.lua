	...
	slot0:on(13001, function (slot0)
		...
		for slot6, slot7 in ipairs(slot0.chapter_list) do
			if not pg.chapter_template[slot7.id] then
				...
				slot8.setEliteFleetList(slot8, uv0.mapEliteFleetCache[slot8:getConfig("map")] or {
					{},
					{},
					{}
				})
				slot8.setEliteCommanders(slot8, uv0.mapEliteCommanderCache[slot9] or {
					{},
					{},
					{}
				})
...
function slot0.getChapterById(slot0, slot1)
	...
	if slot0.data[slot1] then
		...
		if slot2:getConfig("type") == Chapter.CustomFleet then
			...
			-- Decompilation error in this vicinity:
			--- BLOCK #0 55-67, warpins: 2 ---
			slot2.setEliteFleetList(slot2, slot0.mapEliteFleetCache[slot3] or {
				{},
				{},
				{}
			})
			slot2.setEliteCommanders(slot2, slot0.mapEliteCommanderCache[slot3] or {
				{},
				{},
				{}
			})
			...
			if slot16 then
				...
				if slot14.model ~= ChapterConst.TypeMainSub then
					...
					if slot17:getConfig("type") == Chapter.CustomFleet then
						...
						-- Decompilation error in this vicinity:
						--- BLOCK #0 154-154, warpins: 2 ---
						slot17.setEliteFleetList(slot17, slot0.mapEliteFleetCache[slot15.configId] or {
							{},
							{},
							{}
						})
						slot17.setEliteCommanders(slot17, slot0.mapEliteCommanderCache[slot15.configId] or {
							{},
							{},
							{}
						})
