...
function slot0.AddListener(slot0)
	...
	onButton(slot0, slot0.deployBtn, function ()
		...
		slot10:ShowMsgBox({
			content = i18n("tips_confirm_teleport_sub", slot7:TransformLine2PlanePos(slot3), slot8:TransformLine2PlanePos(slot4), slot5, math.ceil(pg.strategy_data_template[ChapterConst.StrategySubTeleport].arg[2] * #slot1:getShips(false) * slot5 - 1e-05)),
			onYes = function ()
				slot0 = uv0
	...
	if slot3 then
		...
		onButton(slot0, slot2, function ()
			...
			slot0:HandleShowMsgBox({
				...
				content = i18n("level_risk_level_mitigation_rate", slot5:getRemainPassCount(), slot6:getMitigationRate())
			})
		end, SFX_PANEL)
	end
end
			...
			onButton(uv0, slot4, function ()
				...
				elseif uv1.id == ChapterConst.StrategySubAutoAttack then
					...
					elseif uv8 == ChapterConst.StgTypeForm then
						...
						slot2:emit(LevelMediator2.ON_OP, {
							type = ChapterConst.OpStrategy,
							id = slot0:getNextStgUser(uv1.id),
...
function slot0.SafeCheck(slot0)
	...
	slot9 = slot0.contextData.map

	if not slot9:isRemaster() and slot8 ~= 0 then
		slot9 = getProxy(ActivityProxy)

		if not slot9:getActivityById(slot8) or slot10:isEnd() then
