...
function slot0.addListener(slot0)
	...
	for slot4 = 1, uv0.Snack_Num, 1 do
		onButton(slot0, slot0.snackContainer:GetChild(slot4 - 1), function ()
			...
			if isActive(slot1:findTF("SelectedTag", uv2)) == true then
...
function slot0.updateSelectedList(slot0, slot1)
	for slot5 = 1, uv0.Order_Num, 1 do
...
function slot0.updateSnackList(slot0, slot1)
	for slot5 = 1, uv0.Snack_Num, 1 do
...
function slot0.openResultView(slot0)
	...
	slot0.packageData = {
		...
		score = slot0.score,
		correctNumToEXValue = slot2:getConfig("simple_config_data").correct_value,
		scoreLevel = slot2:getConfig("simple_config_data").score_level,
