			...
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				...
				onYes = function ()
					...
					if table.getCount(uv0.contextData.materialShipIds or {}) == 0 then
						...
						if not _.all(slot2:hasEliteShips(uv0.contextData.materialShipIds, uv0.shipVOs), function (slot0)
							return slot0 == ""
						end) then
							slot4 = table.concat(slot2, "")
							slot4 = pg.MsgboxMgr.GetInstance()

							slot4:ShowMsgBox({
								content = i18n("destroy_eliteship_tip", string.gsub(slot4, "$1", slot2[1] == "" and "" or ",")),
								onYes = function ()
									uv0:startModShip()
								end
							})
...
function slot0.blurPanel(slot0, slot1)
	if slot1 then
		slot7[MULTRES] = slot0.mainPanel:Find("bg")
...
function slot0.initSelectedShips(slot0)
	for slot6 = 1, uv0, 1 do
		if slot6 <= table.getCount(slot0.contextData.materialShipIds or {}) then
...
function slot0.tweenValue(slot0, slot1, slot2, slot3, slot4, slot5, slot6, slot7, slot8)
	...
	if slot8 and slot8 > 0 then
		slot9.setDelay(slot9, slot5 or 0):setOnComplete(System.Action(function ()
			if uv0 then
				uv0()
			end
		end)):setRepeat(slot8)
