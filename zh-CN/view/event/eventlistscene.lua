		...
		onToggle(slot0, slot0.toggles[slot4], function (slot0)
			...
			if slot0 and uv0.toggleIndex ~= uv1 then
				...
				if slot1 then
					...
					setActive(uv0.listEmptyTF, #uv0.eventList <= 0)
				end
			end
		end)
	end
...
function slot0.showDetail(slot0)
	...
	slot5.anchoredPosition = Vector3.New(-1, -175, 0)
	...
	for slot11 = 0, slot0.scrollRect.content.childCount - 1, 1 do
		slot12 = slot2:GetChild(slot11)

		if slot12:GetComponent(typeof(LayoutElement)).ignoreLayout or not slot12.gameObject.activeSelf then
