...
function slot0.setActivityData(slot0, slot1)
	...
	if not slot0.config_init then
		slot2 = slot0.activityData

		if slot2:getConfig("config_client")[2] then
				...
				table.insert(uv0.cardList, slot1 + 1, CardPairsCard.New(slot2, uv0.pics, uv0.cardIndexList[slot1 + 1][1], function (slot0)
					if uv0.gameState == uv0.GAME_STATE_GAMING then
						...
						if slot0.canClick and slot0.enable and #uv0.showCards < 2 then
							slot0.aniShowBack(slot0, slot0.cardState == CardPairsCard.CARD_STATE_BACK)
						end
					end
					...
					if uv0.gameState == uv0.GAME_STATE_GAMING then
						if slot1 then
							...
							if #uv0.showingCards % 2 == 0 then
								...
								slot4 = uv0.showingCards[slot3]
								slot5 = uv0.showingCards[slot2]

								table.remove(uv0.showingCards, slot2)
								table.remove(uv0.showingCards, slot3)

								if slot4:getCardIndex() == slot5:getCardIndex() then
									slot4:setClear(true)
									slot5:setClear(true)
							...
							slot2.setAllCardEnale(slot2, #uv0.showingCards == 0)
						end
					end
				end))
			end
...
function slot0.gameInit(slot0)
	...
	for slot4 = 1, slot0.CARD_NUM / 2, 1 do
		slot8[MULTRES] = math.random(0, 100)
...
function slot0.tryFirstPlayStory(slot0)
	if slot0.activityData:getConfig("config_client")[1] then
		slot1 = slot0.activityData

		if slot1:getConfig("config_client")[1][1] ~= nil then
			if not slot0.playerData:IsPlayed(slot1) then
