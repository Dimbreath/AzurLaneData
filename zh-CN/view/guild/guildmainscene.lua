...
slot9[MULTRES] = i18n("guild_word_home")
slot9[MULTRES] = i18n("guild_word_member")
slot9[MULTRES] = i18n("guild_word_apply")
slot9[MULTRES] = i18n("word_activity")
slot9[MULTRES] = i18n("word_urgency_event")
slot9[MULTRES] = i18n("word_shop")
slot9[MULTRES] = i18n("word_facility")
...
function slot0.updateEventBtn(slot0, slot1)
	if slot0.toggles then
		slot2 = setActive
		slot3 = slot0.toggles[uv0]

		if slot0.guildEvent then
			slot4 = not slot0.guildEvent:isEnd()
		end
...
function slot0.updateTheme(slot0)
	...
	onButton(slot0, slot0.sendBtn, function ()
		...
		if uv0.chatTimer then
			if pg.TimeMgr.GetInstance():GetServerTime() - uv0.chatTimer < 5 then
...
function slot0.updateAdmin(slot0)
	...
	slot1 = slot0.guildVO
	slot0.announce.interactable = slot1:getDutyByMemberId(slot0.playerVO.id) == GuildMember.DUTY_COMMANDER or slot1 == GuildMember.DUTY_DEPUTY_COMMANDER
...
function slot0.appendWorld(slot0, slot1, slot2)
	...
	if slot2 >= 0 then
		GuildChatBubble.New(cloneTplTo(slot5, slot0.chatContent)).tf:SetSiblingIndex(slot2)
	end
