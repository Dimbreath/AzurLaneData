...
function slot0.didEnter(slot0)
	...
	onToggle(slot0, slot0.sortBtn, function (slot0)
		...
		if uv0.indexPanel then
			...
			slot1 = uv0.scrollRect

			slot1.SetTotalCount(slot1, #uv0.decorationVOs, uv0.contextData.rectValue or 0)
		else
			slot1 = uv0

			slot1.filter(slot1, uv0.contextData.index or 0)
		end
	end, SFX_PANEL)
...
function slot0.initDecorations(slot0)
	...
	slot0.filter(slot0, slot0.contextData.index or 0)
...
function slot0.SetTotalCount(slot0, slot1)
	...
	slot2 = slot0.scrollRect

	slot2.SetTotalCount(slot2, #slot0.decorationVOs, slot0.contextData.rectValue or 0)
	slot0.enableBothBtn(slot0, #slot0.decorationVOs == 0)
...
function slot0.filterAll(slot0, slot1)
	...
	if slot1 == BackYardConst.TAG_INDEX_THEME then
		...
		slot3 = slot0.enableBothBtn

		slot3(slot0, table.getCount(slot3) == 0)
...
function slot0.showFilterPanel(slot0)
	if not slot0.indexPanel then
		...
		function slot0.indexPanel.confirmFunc()
			...
			slot0.SetTotalCount(slot0, uv0.contextData.index or 0)
		end
	end

	slot0.indexPanel:show()
	slot0.filterAll(slot0, slot0.contextData.index or 0)
