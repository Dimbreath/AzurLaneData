...
function slot0.confirm(slot0, slot1)
	if slot1.goods:isChargeType() then
			...
			if not slot1.goods:isGiftBox() then
				...
				elseif slot1.goods:isGem() then
					...
					slot0:showItemDetail({
						isChargeType = true,
						icon = "chargeicon/" .. slot1.goods:getConfig("picture"),
						name = slot11:getConfig("name"),
						price = slot11:getConfig("money"),
						tagType = slot4,
						normalTip = i18n("charge_start_tip", slot6, slot7),
						onYes = function ()
							if uv0:checkSetBirth() then
								uv0:emit(ChargeMediator.CHARGE, uv1.goods.id)
								SetActive(uv0.detail, false)
		...
		slot0:showItemDetail({
			...
			extraItems = slot2,
			price = slot9:getConfig("resource_num"),
			tagType = slot9:getConfig("tag"),
			onYes = function ()
