...
function slot0.createFriendItem(slot0, slot1)
	slot2 = {
		tf = tf(slot1),
		nameTF = slot2.tf:Find("name"):GetComponent(typeof(Text)),
		iconTF = slot2.tf:Find("shipicon/icon"):GetComponent(typeof(Image)),
		circle = slot2.tf:Find("shipicon/frame"),
		toggle = slot2.tf:GetComponent(typeof(Toggle)),
		tipTF = slot2.tf:Find("tip"),
		dateTF = slot2.tf:Find("lv_bg/date"):GetComponent(typeof(Text)),
		onlineTF = slot2.tf:Find("lv_bg/online"),
		levelTF = slot2.tf:Find("lv_bg/Text"):GetComponent(typeof(Text))
	}
...
function slot0.willExit(slot0)
	...
	eachChild(slot0.chatsContainer, function (slot0)
		...
		if slot1:findTF("face", slot0).childCount > 0 then
