...
function slot0.parse_header(slot0)
	slot1, slot2, slot5, slot6 = string.find(string.gsub(string.gsub(slot0, "\n[ ]+", " "), "\n+", ""), "([^%s:]-):%s*(.*)")
...
function slot0.parse_headers(slot0)
	...
	for slot6 = 1, #uv0.split_headers(slot0), 1 do
...
function slot0.split_mbox(slot0)
	...
	while true do
		slot3, slot2 = string.find(string.gsub(slot0, "\r\n", "\n") .. [[
...
function slot0.parse(slot0)
	for slot5 = 1, #uv0.split_mbox(slot0), 1 do
...
function slot0.parse_message(slot0)
	...
	slot1.headers, slot1.body = uv0.split_message(slot0)
