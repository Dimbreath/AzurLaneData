...
function print(...)
	if Application.isEditor then
		...
		uv0(debug.traceback(table.concat(_.map({}, function (slot0)
...
function warning(...)
	if Application.isEditor then
		...
		Debugger.LogWarning(debug.traceback(table.concat(_.map({}, function (slot0)
			return tostring(slot0)
		end), " "), 2))
	else
		uv0(...)
...
function errorMsg(...)
	if Application.isEditor then
		...
		Debugger.LogError(debug.traceback(table.concat(_.map({}, function (slot0)
			return tostring(slot0)
		end), " "), 2))
	else
		uv0(...)
