...
function slot5.AddFX(slot0, slot1, slot2, slot3, slot4)
	if slot2 then
		slot0._cacheFXList[slot1] = slot0:GetFactory():GetFXPool():GetCharacterFX(slot1, slot0, not slot2, function (slot0)
			if uv0 then
				uv0()
			end
...
function slot5.UpdateDiveInvisible(slot0)
	if slot0._unitData:GetDiveInvisible() then
		slot0.updateInvisible(slot0, slot1, slot0._unitData:GetIFF() == uv0.FOE_CODE and "GRID_TRANSPARENT" or "SEMI_TRANSPARENT", slot0:GetFactory():GetDivingFilterColor())
	else
