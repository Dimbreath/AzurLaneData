...
function slot3.onPlayerShutDown(slot0, slot1)
	...
	if #slot0._rivalFleet:GetUnitList() == 0 then
		...
		if slot0._failReason then
			slot5.reason = slot0._failReason
...
function slot3.onUpdateCountDown(slot0, slot1)
	...
	if slot2 <= 0 then
		slot3 = slot0._userFleet
		slot9, slot11 = slot3:GetDamageRatioResult()
		slot5 = slot0._rivalFleet
		slot10, slot12 = slot5:GetDamageRatioResult()
