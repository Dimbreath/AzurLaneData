...
function slot0.Battle.BattleSkillHeal.DoDataEffect(slot0, slot1, slot2)
	if slot0:IsFilterTarget(slot1, slot2) then
		slot2:UpdateHP(math.max(0, math.floor((slot2:GetMaxHP() * slot0._maxHPRatio + math.floor(slot0._number * uv0.Battle.BattleFormulas.HealFixer(uv0.Battle.BattleDataProxy.GetInstance():GetInitData().battleType, slot2:GetAttr()))) * (slot1:GetAttrByName("healingEnhancement") + 1))), {
			isMiss = false,
			isCri = false,
			isHeal = true
		})
