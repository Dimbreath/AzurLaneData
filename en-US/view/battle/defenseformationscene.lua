...
function slot0.loadAllCharacter(slot0)
	...
	function slot1(slot0, slot1, slot2, slot3)
		...
		for slot14 = 1, slot4:getStar(), 1 do
		...
		slot17:AddDragEndFunc(function (slot0, slot1)
			...
			if slot1.position.x < UnityEngine.Screen.width * 0.15 or slot1.position.x > UnityEngine.Screen.width * 0.87 or slot1.position.y < UnityEngine.Screen.height * 0.18 or slot1.position.y > UnityEngine.Screen.height * 0.7 then
				...
				elseif table.getCount(uv0._currentFleetVO.mainShips) == 1 and slot5 == TeamType.Main or table.getCount(uv0._currentFleetVO.vanguardShips) == 1 and slot5 == TeamType.Vanguard then
					slot8.content = i18n("exercise_clear_fleet_tip")
					slot8.onYes = slot3
					slot8.onNo = slot2

					pg.MsgboxMgr.GetInstance():ShowMsgBox({})
				else
					slot8.content = i18n("ship_formationUI_quest_remove", uv5:getName())
					slot8.onYes = slot3
					slot8.onNo = slot2
...
function slot0.resetGrid(slot0, slot1)
	...
	if slot1 == TeamType.Main then
		if #slot0._currentFleetVO:getTeamByName(TeamType.Vanguard) == 0 then
			return
		end
	end
...
function slot0.switchToDisplayMode(slot0)
	function slot1(slot0)
		for slot4, slot5 in ipairs(slot0) do
			slot7 = tf(findTF(slot5, "model"))

			if slot7:Find("mouseChild") then
...
function slot0.sortSiblingIndex(slot0)
	...
	if #slot0._cards[TeamType.Main] > 0 or #slot0._cards[TeamType.Vanguard] > 0 then
...
function slot0.initAttrFrame(slot0)
	slot1[TeamType.Main] = "main"
	slot1[TeamType.Vanguard] = "vanguard"
	slot2[TeamType.Main] = slot0._currentFleetVO.mainShips
	slot2[TeamType.Vanguard] = slot0._currentFleetVO.vanguardShips
	slot3 = false

	for slot7, slot8 in pairs({}) do
		if #slot0._cards[slot7] == 0 then
			for slot14 = 1, 3, 1 do
				table.insert(slot9, FormationCard.New(cloneTplTo(slot0._cardTpl, slot0:findTF(({})[slot7] .. "/list", slot0._attrFrame)).gameObject))
			end
...
function slot0.updateAttrFrame(slot0)
	slot1[TeamType.Main] = slot0._currentFleetVO.mainShips
	slot1[TeamType.Vanguard] = slot0._currentFleetVO.vanguardShips
...
function slot0.getCardAttrProps(slot0, slot1)
	slot6[1] = i18n("word_attr_durability")
	slot6[MULTRES] = tostring(math.floor(slot1:getProperties().durability))
	slot5[1] = {}
	slot6[1] = i18n("word_attr_luck")
	slot6[2] = "" .. tostring(math.floor(slot1:getBattleTotalExpend()))
	slot5[2] = {}
	slot6[1] = i18n("word_synthesize_power")
	slot6[2] = "<color=#ffff00>" .. math.floor(slot1:getShipCombatPower()) .. "</color>"
	slot5[3] = {}
