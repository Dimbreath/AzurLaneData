...
function slot0.displayShips(slot0)
	...
	for slot14, slot15 in ipairs(slot6) do
		...
		if slot3[slot15.id] then
			...
			while slot35 > 0 do
				slot39 = cloneTplTo(slot33, slot32)

				SetActive(slot39:Find("empty"), slot34 < slot35)
			...
			if slot48 and slot0.expBuff then
				setText(slot46, slot0.expBuff:getConfig("name") or slot47 and i18n("Word_Ship_Exp_Buff"))
...
function slot0.showPainting(slot0)
	...
	slot3 = slot0._chat:Find("Text")

	if CHAT_POP_STR_LEN < #slot3:GetComponent(typeof(Text)).text then
