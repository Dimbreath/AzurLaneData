...
function slot0.update_task_list_mikasa_museum(slot0, slot1, slot2, slot3)
	...
	onButton(slot0, slot2:Find("item"):Find("helpBtn"), function ()
		slot2.type = MSGBOX_TYPE_HELP
		slot2.helps = pg.gametip.word_museum_help.tip
	...
	for slot19 = 1, 4, 1 do
		slot12 = slot0:findTF("btn_repair", slot8:Find("Panel/layout_layer/repair_panel" .. slot19 .. "/Panel"))
		slot13[slot19] = nil

		for slot23 = 1, 4, 1 do
			slot0.set_mikasa_btn(slot0, slot5[(slot19 - 1) * 4 + slot23], slot0:findTF("repair" .. slot23, slot10), slot23 == 1 and 0 or slot5[(slot19 - 1) * 4 + slot23 - 1], slot23 >= 4)
	...
	onButton(slot0, slot12, function ()
		if not uv0:isFinish() then
			...
			slot5[1] = slot0:getConfig("config_data")[math.clamp(slot1:DiffDay(slot0.data1, slot1:GetServerTime()) + 1, 1, #slot0:getConfig("config_data"))]
...
function slot0.set_mikasa_btn(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	print(tostring(slot6:getTaskById(slot1) or slot6:getFinishTaskById(slot1)))
	...
	if slot4 then
		...
		slot12.type = slot11[1]
		slot12.id = slot11[2]
		slot12.count = slot11[3]
	...
	if slot7:getConfig("sub_type") == 90 and slot5 then
		for slot13, slot14 in ipairs(slot5) do
			if slot14[1] == slot1 and slot14[2] == slot7:getProgress() then
				if not getProxy(PlayerProxy):getData():IsPlayed(slot14[3]) then
