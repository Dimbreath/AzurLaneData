...
function slot0.OnSendMiniGameOPDone(slot0, slot1)
	...
	if slot2[1] == slot0.miniGameId then
		...
		elseif slot4 == 2 then
			...
			slot8.gold = slot0:GetMGData():getConfig("config_data")[1]
			...
			if slot6:getActivityByType(ActivityConst.ACTIVITY_TYPE_SHRINE) and not slot6:isEnd() then
...
function slot0.initData(slot0)
	...
	function slot3.onSelect(slot0)
		...
		if uv0:GetMGData():GetRuntimeData("count") <= 0 then
			...
			slot8[1] = uv0.miniGameId
			slot8[3] = uv0:GetMGData():getConfig("config_data")[2][slot0]
...
function slot0.addListener(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		slot2.helps = pg.gametip.help_newyear_shrine.tip
		...
		for slot8, slot9 in ipairs(getProxy(PlayerProxy):getData().buff_list) do
			if table.indexof(slot0:GetMGData():getConfig("config_data")[2], slot9.id, 1) then
				if pg.TimeMgr:GetInstance():GetServerTime() < slot9.timestamp then
					setImageSprite(slot0.buffImg, GetSpriteFromAtlas("ui/shrineui_atlas", "buff_type_" .. nil, true))
					setActive(slot0.buffImg, true)
...
function slot0.updateBuffDesc(slot0)
	...
	if slot2:getActivityByType(ActivityConst.ACTIVITY_TYPE_MINIGAME) and not slot2:isEnd() then
