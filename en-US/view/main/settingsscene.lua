	...
	if SFX_TEST then
		...
		function slot1()
			...
			if slot1[1] == "skill" then
				...
				pg.CriMgr.GetInstance():PlaySoundEffect_V3(Ship.getCVPath(tonumber(uv0._cvID:GetComponent(typeof(InputField)).text), slot1[1]), function (slot0)
					uv0._currentVoice = slot0
				end)
			else
				slot4:GetComponent(typeof(Text)).text, slot3 = Ship.getWords(slot0, slot1[2], slot1[3])
				slot4 = uv0._cvChat
		...
		onButton(slot0, slot0._cvPlay, function ()
			...
			if uv0._cvList[Ship.getCVKeyID(tonumber(uv0._cvID:GetComponent(typeof(InputField)).text))] then
				uv1()
			else
