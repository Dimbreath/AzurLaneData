...
function slot0.UpdateView(slot0)
	...
	for slot7 = 1, #slot0.taskGroup, 1 do
		...
		if slot0.taskProxy:getTaskVO(slot9) then
			...
			setText(slot0:findTF("desc", slot8), string.gsub(string.gsub(pg.task_data_template[slot9].desc, "$1", (slot14 and pg.task_data_template[slot9].target_num) or 0), "$2", pg.task_data_template[slot9].target_num))
