...
function slot1.Share(slot0, slot1, slot2)
	if not CheckPermissionGranted(ANDROID_WRITE_EXTERNAL_PERMISSION) then
		slot5.content = i18n1("指揮官，碧藍航向需要存儲權限才能分享是否打開？")

		function slot5.onYes()
			slot1[1] = ANDROID_WRITE_EXTERNAL_PERMISSION
	...
	slot5 = slot2 == uv1.PANEL_TYPE_PINK

	setActive(slot0.panelPink, slot5)
	...
	_.each(slot3.move_comps, function (slot0)
		if not IsNil(GameObject.Find(slot0.path)) then
			slot8[1] = slot1
			slot8[2] = slot1.transform.anchoredPosition.x
			slot8[3] = slot1.transform.anchoredPosition.y
