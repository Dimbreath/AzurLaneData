...
function slot1.Reconnect(slot0, slot1)
	...
	slot0:Connect(slot0:GetLastHost(), slot0:GetLastPort(), function ()
		...
		slot6.platform = slot2
		slot6.account_id = slot1.uid
		slot6.server_ticket = slot1.token
		slot6.serverid = slot1.server
		slot6.check_key = HashUtil.CalcMD5(slot1.token .. AABBUDUD)
		slot6.device_id = uv0.SdkMgr.GetInstance():GetDeviceId()

		uv1:Send(10022, {}, 10023, function (slot0)
...
function slot1.onError(slot0)
	...
	if uv6 and uv7 then
		...
		if table.contains({
			...
			slot6.content = i18n("reconnect_tip", slot0)
