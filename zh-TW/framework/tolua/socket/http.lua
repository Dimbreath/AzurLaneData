...
function slot11(slot0, slot1)
	...
	while slot2 ~= "" do
		slot3, slot4 = uv0.skip(2, uv1.find(slot2, "^(.-):%s*(.*)"))
...
slot0.sourcet["http-chunked"] = function (slot0, slot1)
	function slot3.getfd()
		return uv0:getfd()
	end

	function slot3.dirty()
		return uv0:dirty()
	end

	function slot4.__call()
...
slot0.sinkt["http-chunked"] = function (slot0)
	function slot2.getfd()
		return uv0:getfd()
	end

	function slot2.dirty()
		return uv0:dirty()
	end
...
function slot8.open(slot0, slot1, slot2)
	...
	slot5.c = slot3
	slot4 = uv1.setmetatable({}, uv2)
	slot4.try = uv0.newtry(function ()
		uv0:close()
	end)

	slot4.try(slot3:settimeout(uv3.TIMEOUT))
	slot4.try(slot3.connect(slot3, slot0, slot1 or uv4))
...
function slot12.__index.sendrequestline(slot0, slot1, slot2)
	...
	return slot0.try(slot5:send(uv0.format("%s %s HTTP/1.1\r\n", slot5, slot2)))
end
...
function slot13(slot0)
	...
	if not slot0.proxy and not uv0.PROXY then
		slot2.path = uv1.try(slot0.path, "invalid path 'nil'")
		slot2.params = slot0.params
		slot2.query = slot0.query
		slot2.fragment = slot0.fragment
		slot1 = {}
	end
...
function slot17(slot0)
	for slot5, slot6 in uv2.pairs(slot0) do
		slot0.url and uv0.parse(slot0.url, uv1) or {}[slot5] = slot6
	end
...
function slot21(slot0, slot1)
	slot3.url = uv1.absolute(slot0.url, slot1)
	slot3.source = slot0.source
	slot3.sink = slot0.sink
	slot3.headers = slot0.headers
	slot3.proxy = slot0.proxy
	slot3.nredirects = (slot0.nredirects or 0) + 1
	slot3.create = slot0.create
...
function slot8.genericform(slot0, slot1)
	...
	if slot1 then
		...
		slot4["content-length"] = uv1.len(slot1)
		slot3.headers = {
...
function slot23(slot0, slot1)
	...
	slot3, slot8, slot9, slot10 = uv1(slot2)
