...
function slot0.updateExtendItemVO(slot0, slot1)
	if not slot1:getItemById(ITEM_BACKYARD_AREA_EXTEND) then
...
function slot0.OnDidEnter(slot0)
	...
	onButton(slot0, slot0.backBtn, function ()
		...
		if uv0.houseVO:hasChangeFurnitures() then
			...
			slot2.content = i18n("backyard_backyardScene_quest_saveFurniture")

			function slot2.onYes()
				uv0:save()
			end

			slot2.yesSound = SFX_FURNITRUE_SAVE
	...
	onButton(slot0, go(slot0.road), function ()
		...
		function slot4()
			if uv0.itemVO.count <= 0 then
				slot2.content = i18n("backyard_buyExtendItem_question", uv1.resource_num .. uv2)
		...
		for slot10, slot11 in ipairs(string.split(i18n("backyard_extendArea_tip", 1, uv1.itemVO.count), "||")) do
	...
	slot3:SetAfterFunc(function (slot0)
		for slot5, slot6 in ipairs(slot0) do
			if not slot6.ob.isBoat then
				uv0.furnitureModals[slot6.ob.id]:SetSiblingIndex(0)
			end
...
function slot0.getMap(slot0, slot1)
	...
	elseif slot1.parent ~= 0 and not slot0.maps[slot1.parent] then
		slot2 = slot0.furnitureVOs[slot1.parent]
		slot8, slot9 = slot2:getMapSize()
...
function slot0.initFurnitures(slot0)
	...
	slot4[4] = function (slot0)
		limitedParallelAsync(uv0[3], 5, slot0)
	end

	slot4[5] = function (slot0)
		seriesAsync(uv0[4], slot0)
	end

	slot4[6] = function (slot0)
...
function slot0.furnitureDrag(slot0, slot1, slot2)
	...
	if not slot2:isFloor() then
		if not slot0.houseVO:isLimitWallBound(slot2, slot0.houseVO:limitWallFurnWidth(slot0:getMapPos(slot1), slot2)) then
			return
		end
	end
...
function slot0.furnitureEndDrag(slot0, slot1, slot2)
	...
	if not slot2:isFloor() then
		if not slot0.houseVO:isLimitWallBound(slot2, slot0.houseVO:limitWallFurnWidth(slot1, slot2)) then
	...
	if function (slot0, slot1)
		...
		function ()
			...
			slot1.emit(slot1, BackyardMainMediator.FURNITURE_POS_CHNAGE, uv1.id, uv2, slot0:isLocaledAndPutOn() and slot0.id or slot0)
			uv3()
		end()
	else
		slot7(slot2)
...
function slot0.OnWillExit(slot0)
	...
	for slot5, slot6 in pairs(slot0.furnitureModals) do
		slot7 = slot0.furnitureVOs[slot5]

		if slot7:hasParent() and not slot7:hasChild() then
			table.insert(({})[1], slot6)
