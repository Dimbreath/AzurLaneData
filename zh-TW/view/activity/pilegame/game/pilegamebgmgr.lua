...
function slot0.Init(slot0, slot1)
	slot2[1] = slot0.tr:Find("Image1")
	slot2[2] = slot0.tr:Find("Image2")
	slot2[MULTRES] = slot0.tr:Find("Image3")
	slot0.list = {}
	slot0.names = {}
...
function slot0.DoMove(slot0, slot1)
	for slot6, slot7 in ipairs(slot0.list) do
		if slot7 then
			slot2 = nil or slot6
			slot11.y = getAnchoredPosition(slot7).y - slot1
...
function slot0.DoCheck(slot0, slot1)
	...
	if slot2.rect.height <= math.abs(slot4.y) then
		...
		slot9.y = getAnchoredPosition(slot3).y + slot3.rect.height
