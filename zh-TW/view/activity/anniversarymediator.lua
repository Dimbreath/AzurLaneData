...
function slot0.acceptTask(slot0, slot1)
	...
	if slot1.data3 ~= 0 and slot6 < math.clamp(slot4:DiffDay(slot1.data1, slot4:GetServerTime()) + 1, 1, #slot1:getConfig("config_data")) then
		slot9[1] = slot3[slot6]

		if _.all(_.flatten({}), function (slot0)
			return uv0:getFinishTaskById(slot0) ~= nil
		end) then
			slot10.activity_id = slot1.id
...
function slot0.listNotificationInterests(slot0)
	...
	slot1[4] = TaskProxy.TASK_FINISH
	slot1[5] = GAME.SUBMIT_TASK_DONE
	slot1[6] = ActivityProxy.ACTIVITY_UPDATED
