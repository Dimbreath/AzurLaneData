...
function slot0.init(slot0)
	...
	slot2(slot0._fleetInfo, slot0.contextData.system ~= SYSTEM_DUEL)
...
function slot0.SetStageID(slot0, slot1)
	...
	if checkExist(pg.expedition_activity_template[slot1], {
		...
		for slot9 = #slot4, 1, -1 do
			if getProxy(ActivityProxy):getActivityById(slot4[slot9][1]) and not slot10:isEnd() then
				slot14[MULTRES] = id2ItemId(slot4[slot9][2])
	...
	slot6[1] = findTF(slot0._goals, "goal_tpl")
	slot6[2] = findTF(slot0._goals, "goal_sink")
	slot6[MULTRES] = findTF(slot0._goals, "goal_time")
	slot7[1] = slot2.objective_1
	slot7[2] = slot2.objective_2
	slot7[3] = slot2.objective_3
	slot8 = 1
...
function slot0.didEnter(slot0)
	onButton(slot0, slot0._backBtn, function ()
		if uv0._currentForm == uv1.FORM_EDIT and uv0._editedFlag then
			slot2.content = i18n("battle_preCombatLayer_save_confirm")
	...
	onButton(slot0, slot0._startBtn, function ()
		if uv0._currentForm == uv1.FORM_EDIT and uv0._editedFlag then
			slot2.content = i18n("battle_preCombatLayer_save_march")
...
function slot0.enabledCharacter(slot0, slot1, slot2, slot3, slot4)
	if slot2 then
			...
			slot8:AddDragEndFunc(function (slot0, slot1)
				...
				if uv1.contextData.system ~= SYSTEM_HP_SHARE_ACT_BOSS and uv1.contextData.system ~= SYSTEM_ACT_BOSS and uv1.contextData.system ~= SYSTEM_BOSS_EXPERIMENT and (slot1.position.x > UnityEngine.Screen.width * 0.65 or slot1.position.y < UnityEngine.Screen.height * 0.25) then
					if not uv1._currentFleetVO:canRemove(uv2) then
						...
						slot5.content = i18n("battle_preCombatLayer_quest_leaveFleet", uv2:getConfigTable().name)
		...
		if not IsNil(slot5:Find("mouseChild")) then
