...
function slot0.didEnter(slot0)
	...
	onToggle(slot0, slot0.toggles[uv0.TYPE_FRAGMENT], function (slot0)
		if slot0 then
			...
			slot1:updatePainting(uv1.TYPE_FRAGMENT)
		end
	...
	if not LOCK_FRAGMENT_SHOP then
		if slot0.contextData.shopDatas.fragshop then
			slot6 = slot0.contextData.shopDatas.fragshop
			slot6 = slot6:isOpen()
		end
	else
		slot6 = false
	...
	setActive(slot0.toggles[uv0.TYPE_FRAGMENT], slot6)
	onToggle(slot0, slot0.toggles[uv0.TYPE_FRAGMENT], function (slot0)
		if slot0 then
			slot1 = pg.SystemOpenMgr.GetInstance()
	...
	onButton(slot0, slot9:Find("res_fragment/resolve"), function ()
		slot0 = uv0
	...
	if slot0.warp == uv0.TYPE_SHAM_SHOP then
		slot7 = pg.SystemOpenMgr.GetInstance()
		slot7, slot8 = slot7:isOpenSystem(slot0.player.level, "ShamShop")

		if not slot7 then
			slot0.warp = nil
			slot9 = pg.TipsMgr.GetInstance()

			slot9:ShowTips(slot8)
		end
		...
		if not slot4 then
			...
			slot9:ShowTips(i18n("common_activity_not_start"))
		end
	end
