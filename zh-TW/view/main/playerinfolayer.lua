...
function slot0.OffsetSource(slot0, slot1, slot2)
	...
	slot12.x = -(slot3.rect.width * slot0.rightPanel.localScale.x + slot2 + slot8)
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		slot2.helps = i18n("secretary_help")
...
function slot0.showCharacters(slot0)
	...
	onToggle(slot0, slot0.hzszBtn, function (slot0)
		...
		setActive(uv0:findTF("setting_off", uv0.hzszBtn), not slot0)
		...
		if slot0 then
			...
			for slot4 = 1, uv0.secretary_max, 1 do
...
function slot0.openChangePlayerNamePanel(slot0)
	if not slot0.changeNamePanel then
		slot0:initChangePlayerNamePanel(function ()
			...
			if slot0:getModifyNameComsume()[1] == DROP_TYPE_RESOURCE then
				slot4.id = id2ItemId(slot0[2])
...
function slot0.uiStartAnimating(slot0)
	...
	shiftPanel(slot4, 0, nil, nil, 0.35, true, true)
end
...
function slot0.initPlayerInfo(slot0)
	...
	slot5[1] = i18n("friend_resume_title_metal") .. pg.arena_data_rank[slot2].name
	slot5[2] = slot0.fleetGS
	slot5[3] = slot0.collectionRate * 100 .. "%"
	slot6 = {
		slot1.shipCount,
		slot1.attackCount,
	...
	setText(findTF(slot7, "title/exp"), slot0.player.exp .. "/" .. getConfigFromLevel1(pg.user_level, slot1.level).exp)

	for slot14, slot15 in ipairs({}) do
...
function slot0.getCardAttrProps(slot0, slot1)
	...
	slot7[2] = slot1.level
	slot6[1] = {}
	slot7[1] = i18n("attribute_intimacy")
	slot6[2] = {}
	slot7[1] = i18n("word_synthesize_power")
	slot7[2] = "<color=#ffff00>" .. math.floor(slot1:getShipCombatPower()) .. "</color>"
	slot6[3] = {}
