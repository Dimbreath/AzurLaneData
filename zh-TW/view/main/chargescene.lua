...
function slot0.initDamonds(slot0)
	...
	slot0.giftRect = slot0:initRect(slot0.viewContainer:Find("giftPanel"), function (slot0)
		...
		onButton(slot0, slot1.tr, function ()
			...
			-- Decompilation error in this vicinity:
			--- BLOCK #0 1-4, warpins: 1 ---
			if slot0.isAlphaTest == 1 then

				-- Decompilation error in this vicinity:
				--- BLOCK #0 5-13, warpins: 1 ---
				pg.TipsMgr.GetInstance():ShowTips("本次測試無法使用充值功能")
				--- END OF BLOCK #0 ---
		...
		if slot1.goods:isMonthCard() or slot1.goods:isGiftBox() or slot1.goods:isItemBox() then
			...
			if slot1.goods:isGem() then
				...
				slot0:showItemDetail({
					...
					price = slot1.goods:getConfig("display_money"),
					tagType = slot4,
					normalTip = i18n("charge_start_tip", slot1.goods:getConfig("display_money"), (slot3 and slot1.goods:getConfig("gem") + slot1.goods:getConfig("gem")) or slot1.goods.getConfig("gem") + slot1.goods:getConfig("extra_gem")),
					onYes = function ()
