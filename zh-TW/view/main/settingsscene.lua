...
function slot0.initResDownloadPanel(slot0, slot1)
	...
	if CSharpVersion >= 35 then
		...
		onButton(slot0, slot0.repairBtn, function ()
			slot2.content = i18n("resource_verify_warn")
...
function slot0.initSoundPanel(slot0, slot1)
	...
	onButton(slot0, slot0.revertBtn, function ()
		slot2.content = i18n("sure_resume_volume")
...
slot1 = {}
slot2 = {}
slot3.title = i18n("words_show_ship_name_label")
slot3.desc = i18n("option_desc1")
slot2[1] = {}
slot3.title = i18n("words_auto_battle_label")
slot3.name = AUTO_BATTLE_LABEL
slot3.desc = i18n("option_desc2")
slot2[2] = {}
slot3.title = i18n("words_rare_ship_vibrate")
slot3.name = RARE_SHIP_VIBRATE
slot3.desc = i18n("option_desc3")
slot2[3] = {
	default = 1
}
slot3.title = i18n("words_display_ship_get_effect")
slot3.name = DISPLAY_SHIP_GET_EFFECT
slot3.desc = i18n("option_desc4")
slot2[4] = {
	default = 1
}
slot3.title = i18n("words_show_touch_effect")
slot3.name = SHOW_TOUCH_EFFECT
slot3.desc = i18n("option_desc5")
slot2[5] = {
	default = 1
}
slot3.title = i18n("words_bg_fit_mode")
slot3.name = BG_FIT_MODE
slot3.desc = i18n("option_desc6")
slot2[6] = {
	default = 0
}
...
function slot0.initInterfacePreference(slot0, slot1)
	...
	onButton(slot0, slot0.revertBtn, function ()
		slot2.content = i18n("setting_interface_revert_check")
	...
	onButton(slot0, slot0.cancelBtn, function ()
		...
		slot2.content = i18n("setting_interface_cancel_check")
	...
	slot0:initInterfaceComponent(slot0.skillBtn4, "skill_4_anchorX", "skill_4_anchorY", slot9[4])

	slot11[1] = findTF(slot0.interface, "top")
	slot11[2] = slot0.stick
	slot11[3] = slot0.skillBtn1
	slot11[4] = slot0.skillBtn2
	slot11[5] = slot0.skillBtn3
	slot11[6] = slot0.skillBtn4
	slot0.components = {}
	...
	for slot8, slot9 in ipairs(slot0.components) do
		for slot13, slot14 in ipairs(slot0.components) do
			if slot9 ~= slot14 then
				if slot3[slot9]:Overlaps(slot3[slot14]) then
					slot1[slot14] = true
				end
			end
		end
...
function slot0.updateSoundDownloadState(slot0)
	...
	slot11.x = slot2 == DownloadState.CheckToUpdate and 82.5 or 91.5
...
function slot5()
	...
	if not uv0 then
		slot1 = {}
		slot3.title = i18n("words_settings_unlock_ship")
		slot1[slot0.UNLOCK_SHIP] = {}
		slot3.title = i18n("words_settings_resolve_equip")
		slot1[slot0.RESOLVE_EQUIPMENT] = {}
		slot3.title = i18n("words_settings_unlock_commander")
		slot1[slot0.UNLOCK_COMMANDER] = {}
		slot3.title = i18n("words_settings_create_inherit")
		slot1[slot0.CREATE_INHERIT] = {}
	end
...
function slot0.initOtherPanel(slot0)
	...
	if slot1 then
		...
		onButton(slot0, slot0.achieveBtn, function ()
			slot3.key = uv0.codeInput:GetComponent(typeof(InputField)).text
	...
	if 2 < Screen.width / Screen.height - 0.001 then
		...
		setSlider(slot0.notchSlider, 2, slot3, getProxy(SettingsProxy):GetScreenRatio())
		slot0:initSoundSlider(slot0.notchSlider, function (slot0)
			uv0:SetScreenRatio(slot0)
	...
	onButton(slot0, slot0:findTF("secondpwd/btnhelp", slot0.otherContent), function ()
		...
		slot2.helps = i18n("secondary_password_help")

		pg.MsgboxMgr.GetInstance():ShowMsgBox({})
	end)

	slot7 = slot0:findTF("secondpwd", slot0.otherContent)
	slot12 = getProxy(SecondaryPWDProxy):getRawData()

	onButton(slot0, slot7:Find("options/close"), function ()
		if uv0.state > 0 then
	...
	onButton(slot0, slot7:Find("options/open"), function ()
		if uv0.state <= 0 then
			function slot0()
			...
			if PlayerPrefs.GetFloat("firstOpenSecondaryPassword") == 0 then
				...
				slot3.helps = i18n("secondary_password_help")
				slot3.onYes = slot0
				slot3.onClose = slot0
	...
	for slot18, slot19 in ipairs(uv0()) do
		if table.contains(pg.SecondaryPWDMgr.GetInstance().LIMITED_OPERATION, slot18) then
			slot20 = cloneTplTo(slot0:findTF("notify_tpl"), slot0:findTF("limited_operations/options", slot0.otherContent))
			slot0.secPwdOpts[slot18] = slot20

			setText(slot0:findTF("Text", slot20), slot19.title)
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.logoutButton, function ()
		slot2.content = i18n("main_settingsScene_quest_exist")
	...
	slot0:onAddToggleEvent(slot0, slot0.otherToggle, function (slot0)
		if slot0 and PlayerPrefs.GetFloat("firstIntoOtherPanel") == 0 then
			...
			slot6.x = slot2.anchoredPosition.x
			slot6.y = -uv0:findTF("secondpwd", slot2).anchoredPosition.y
	...
	if SFX_TEST then
		...
		function slot1()
			...
			if slot1[1] == "skill" then
				slot2, slot7, slot4 = ShipWordHelper.GetWordAndCV(tonumber(uv0._cvID:GetComponent(typeof(InputField)).text), slot1[1])
				uv0._cvChat:GetComponent(typeof(Text)).text = ""
				uv0._currentVoice = playSoundEffect(slot3)
			else
				slot2, slot7, slot5:GetComponent(typeof(Text)).text = ShipWordHelper.GetWordAndCV(slot0, slot1[2], slot1[3])
				slot5 = uv0._cvChat
				uv0._currentVoice = playSoundEffect(slot3)
			end
		end
		...
		onButton(slot0, slot0:findTF("dungeonHPPanel/calcBtn"), function ()
			slot1 = uv0._dungeonID
			slot2 = 0
			slot3 = {}

			for slot8, slot9 in ipairs(ys.Battle.BattleDataFunction.GetDungeonTmpDataByID(tonumber(tonumber(uv0._dungeonLV:GetComponent(typeof(InputField)).text):GetComponent(typeof(InputField)).text)).stages) do
...
function slot0.UpdateTwAccountPanel(slot0)
	...
	slot1[1] = slot0.accountTw:Find("page1/bind_facebook")
	slot1[2] = slot0.accountTw:Find("page1/bind_google")
	slot1[3] = slot0.accountTw:Find("page1/bind_phone")
	slot1[MULTRES] = slot0.accountTw:Find("page1/bind_gamecenter")
	slot2 = pg.SdkMgr.GetInstance()
	slot7[1] = slot2:IsBindFaceBook()
	slot7[2] = slot2:IsBindGoogle()
	slot7[3] = slot2:IsBindPhone()
	slot7[4] = slot2:IsBindGameCenter()
