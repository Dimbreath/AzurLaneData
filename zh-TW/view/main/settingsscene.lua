...
function slot0.initOtherPanel(slot0)
	...
	if 2 < Screen.width / Screen.height - 0.001 then
		...
		setSlider(slot0.notchSlider, slot5, slot3, getProxy(SettingsProxy):GetScreenRatio())
		slot0:initSoundSlider(slot0.notchSlider, function (slot0)
			slot0:SetScreenRatio(slot0)
	...
	slot10 = pg.SecondaryPWDMgr.GetInstance()
	slot12 = getProxy(SecondaryPWDProxy).getRawData(slot11)
	...
	slot13 = slot0:findTF("limited_operations/options", slot0.otherContent)
	slot14 = slot0:findTF("notify_tpl")
	slot0.secPwdOpts = {}

	for slot18, slot19 in ipairs(slot0()) do
		if table.contains(slot10.LIMITED_OPERATION, slot18) then
			slot20 = cloneTplTo(slot14, slot13)
