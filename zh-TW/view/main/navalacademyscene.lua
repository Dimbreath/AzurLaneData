...
function slot0.updateResourceBuilding(slot0, slot1, slot2, slot3)
	...
	if slot3._type == ResourceField.TYPE_CLASS then
		if slot0._courseVO:inClass() then
			if AcademyCourse.MaxStudyTime <= pg.TimeMgr.GetInstance():GetServerTime() - slot0._courseVO.timestamp then
...
function slot0.OpenResourcePanel(slot0, slot1)
	...
	slot3 = slot0._resourceLayer:Find("upgrade_btn")
	...
	if slot11 == slot12 then
		...
		slot3:Find("cost"):GetComponent("Text").text = "-"
...
function slot0.loadCharacter(slot0, slot1, slot2, slot3)
	function slot4(slot0)
		...
		slot1 = slot0:GetComponent("SpineAnimUI")

		slot1.SetAction(slot1, uv2 or "stand", 0)
...
function slot0.displayShipWord(slot0, slot1, slot2)
	if not slot0._chatFlag then
		...
		slot6 = slot0._chatText

		if CHAT_POP_STR_LEN_SHORT < #slot6:GetComponent(typeof(Text)).text then
	...
	for slot6, slot7 in pairs(slot1) do
		if not slot0.academyStudents[slot6] then
			...
			slot9:setCallBack(function (slot0)
				...
				slot2, slot3 = uv0:getStudents()

				uv0:onTask(uv2, slot3[uv1])
			end)
	...
	if _.detect(slot4:getActivitiesByType(ActivityConst.ACTIVITY_TYPE_TASK_LIST), function (slot0)
		...
		if slot6.id == ActivityConst.JYHZ_ACTIVITY_ID and slot2.acceptTaskId then
			...
			if slot4:getActivityByType(ActivityConst.ACTIVITY_TYPE_ZPROJECT) and _.detect(slot8:getConfig("config_data"), function (slot0)
				...
				if getProxy(ChapterProxy):getChapterById(slot10).active then
		...
		elseif not slot2.currentTask and slot2.acceptTaskId then
			...
			slot13.activity_id = slot6.id
			slot13.arg1 = slot2.acceptTaskId

			slot0:emit(NavalAcademyMediator.ACTIVITY_OP, {
				cmd = 1
			})
		elseif slot2.currentTask then
			if not slot2.currentTask:isFinish() then
				if slot2.currentTask:getConfig("sub_type") == 29 then
					slot10.taskVO = slot2.currentTask
