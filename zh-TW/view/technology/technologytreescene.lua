...
function slot0.addBtnListener(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		if pg.gametip.help_technologytree then
			...
			slot2.helps = pg.gametip.help_technologytree.tip
...
function slot0.updateShipItemList(slot0, slot1, slot2)
	...
	slot4:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setText(uv0:findTF("NameText", uv0:findTF("NameBG", slot2)), shortenString(ShipGroup.getDefaultShipNameByGroupID(slot26), 6))
			...
			if table.indexof(uv0.groupIDGotList, slot26, 1) then
				...
				slot31 = uv0.collectionProxy

				if slot31:getShipGroup(slot26).maxLV < 120 then
