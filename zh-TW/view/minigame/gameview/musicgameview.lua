...
function slot0.MyGetRuntimeData(slot0)
	...
	for slot4 = 1, slot0.music_amount * 2, 1 do
		slot8[1] = slot4 + 2
		slot0.bestScorelist[slot4] = checkExist(slot0:GetMGData():GetRuntimeData("elements"), {}) or 0
	end
end

function slot0.MyStoreDataToServer(slot0)
	slot1[1] = slot0.achieve_times
	...
	for slot5 = 1, slot0.music_amount * 2, 1 do
		table.insert({
			nil,
			1
		}, slot5 + 2, slot0.bestScorelist[slot5])
	end
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.pauseview:Find("bottom/back"), function ()
		slot2.content = i18n("reselect_music_game")
	...
	onButton(slot0, slot0.pauseview:Find("bottom/restart"), function ()
		slot2.content = i18n("restart_music_game")
...
function slot0.OnApplicationPaused(slot0, slot1)
	if slot1 and not slot0.countingfive_flag and not slot0.firstview_timerRunflag and slot0.game_playingflag then
		if not isActive(slot0.top:Find("pause_above")) then
			triggerButton(slot0.top:Find("pause"))
		end
	end
	...
	function slot1()
		...
		function slot3(slot0, slot1, slot2, slot3, slot4)
			if slot1 ~= null and tonumber(slot1.begin_time) - slot2 <= uv0.musicgame_nowtime then
				...
				slot6.ob = slot5
				slot6.begin_time = slot1.begin_time
				slot6.end_time = slot1.end_time
				slot6.key_flag = slot1.key_flag
		...
		function slot4(slot0, slot1, slot2, slot3)
			...
			if slot0 == "left" and uv0.piece_nowl.ob and uv0.piece_nowl.begin_time ~= uv0.piece_nowl.end_time then
				if uv0.piece_nowl.key_flag == "K_BOTH" then
					...
					slot6 = Color.New
					uv0.piece_nowl.ob:Find(uv0.game_dgree .. "/notelong_2/note_above/above"):GetComponent(typeof(Image)).color = slot6(1, 1, 1, uv0.piece_nowl_downflag and 1 or 0.8)
					...
					slot6 = Color.New
					uv0.piece_nowl.ob:Find(uv0.game_dgree .. "/notelong_1/note_above/above"):GetComponent(typeof(Image)).color = slot6(1, 1, 1, uv0.piece_nowl_downflag and 1 or 0.8)
			...
			if slot0 == "right" and uv0.piece_nowr.ob and uv0.piece_nowr.begin_time ~= uv0.piece_nowr.end_time then
				if uv0.piece_nowl.key_flag == "K_BOTH" then
					...
					slot6 = Color.New
					uv0.piece_nowr.ob:Find(uv0.game_dgree .. "/notelong_2/note_above/above"):GetComponent(typeof(Image)).color = slot6(1, 1, 1, uv0.piece_nowr_downflag and 1 or 0.8)
					...
					slot6 = Color.New
					uv0.piece_nowr.ob:Find(uv0.game_dgree .. "/notelong_1/note_above/above"):GetComponent(typeof(Image)).color = slot6(1, 1, 1, uv0.piece_nowr_downflag and 1 or 0.8)
...
function slot0.score_update(slot0, slot1)
	for slot6 = 1, 3, 1 do
		setActive(slot0.game_content:Find("evaluate"):GetChild(slot6 - 1), false)
	end
...
function slot0.showSelevtView(slot0)
	...
	onButton(slot0, slot6:Find("help_btn"), function ()
		...
		slot2.helps = pg.gametip.help_music_game.tip
...
function slot0.changeLocalpos(slot0, slot1)
	...
	slot0.changeLocalpos_timer = Timer.New(function ()
		...
		for slot3 = 1, uv1.music_amount, 1 do
			slot4 = slot3 + uv2
...
function slot0.locadScoreView(slot0)
	...
	slot5 = slot0:GetMGData()
	slot6 = slot5:getConfig("simple_config_data").story[slot0:GetMGHubData().usedtime + 1] and slot5[slot3.usedtime + 1][1] or nil
