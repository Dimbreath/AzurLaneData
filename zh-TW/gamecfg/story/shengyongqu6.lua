return {
	fadeType = 1,
	mode = 2,
	once = true,
	id = "SHENGYONGQU6",
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			dir = 1,
			blackBg = true,
			say = "莫桑比克海峡·圣堂中央",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			dir = 1,
			blackBg = true,
			say = "在自由鸢尾和皇家的联合舰队与塞壬展开战斗的同时----",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			nameColor = "#ffff4d",
			side = 2,
			bgName = "bg_qiongding_3",
			dir = 1,
			bgmDelay = 0.5,
			bgm = "bgm-cccp2",
			actorName = "沃克兰",
			say = "警警警报！自由鸢尾和皇家的联合舰队刚刚驶入莫桑比克海峡，已经与封锁海峡的塞壬舰队发生战斗了！",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "果然按时来了啊，这次教廷的情报倒是意外的准确嘛。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "也不能总像之前一样一而再再而三吃亏嘛。由枢机主教黎塞留亲自带队，再加上皇家的大贵族光辉。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "为了到我们这个偏远的圣堂“做礼拜”，还真是来了一个豪华阵容啊…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "唔……要是就这么把圣堂丢了，我们可就麻烦大了哎…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "沃克兰",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "阿尔及利亚、拉.加利索尼埃，妳们难道不觉得奇怪么？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "沃克兰",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "这里的圣堂就算再重要也不过是栋有历史意义的建筑而已吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "沃克兰",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "明明欧罗巴大陆现在更加危险，为什么不调我们回去防卫本土，反而增派了援军过来陪我们一起看风景啊…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "可是黎塞留真的乖乖带着舰队兴师动众的绕了一大圈过来找我们玩了。应该说教廷司令部难得的料事如神呢，还是说…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "阿尔及利亚，妳知道我们现在保卫的圣堂究竟隐藏着什么能让黎塞留感兴趣的东西么？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "不知道。我收到的命令很简单，不惜代价保护圣堂，击退或消灭一切入侵者。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "妳也什么都不知道么…其实我因为太好奇，已经偷偷调查过一轮啦。至于调查的结果，妳们猜怎么样？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "我不好奇。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			nameColor = "#ffff4d",
			side = 2,
			bgName = "bg_qiongding_3",
			dir = 1,
			actorName = "沃克兰",
			say = "我很好奇！",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "令人震惊的是，就算利用我在审判庭的关系查到的蛛丝马迹，居然也都是最高机密！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "沃克兰",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "……………………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "算啦，就算是阻止枢机主教拿回她落在圣堂的旧袍子，我们也得拼命战斗才是…还是想想怎么应付眼前的攻击吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "沃克兰",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "……要是封锁海峡的塞壬能就这么把黎塞留她们挡在外面就好了呢。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "那群杂鱼连我都挡不住，怎么可能挡住黎塞留的联合舰队啊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "不过嘛，作为第一波炮灰牺牲掉倒是不错。它们倒是还是真听话，不让它们靠近圣堂周边，它们就真的不靠过来了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "塞壬能这么听从铁血的命令，反而令我感到非常恐怖。铁血现在究竟在和塞壬做些什么…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "妳这个问题现在该是，教廷和铁血以及塞壬究竟在做些什么才对了…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_qiongding_3",
			dir = 1,
			say = "拉.加利索尼埃往后瞥了一眼，在远方的高墙下，一个强大的战争机器仿佛一具雕塑一般，静静地矗立着。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "…那个教廷派来“秘密兵器”妳也看到了，她真的是「舰船」么？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "黎塞留级战列舰四号舰加斯科涅。我可没听说过黎塞留级存在有一艘四号舰啊！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "从递交了司令部的证明文件之后就一言不发的立在那里，不与我们交流，甚至不听从妳这个圣堂舰队指挥的命令。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "拉.加利索尼埃",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "从我的个人经验来说，这怎么看都不是前来支援的“援军”的架势啊……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "这种情况下就算一艘量产型也很宝贵…有了总比没有好，我们也没有选择的余地了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "阿尔及利亚",
			side = 2,
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "全员注意，下面开始进行作战部署。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
