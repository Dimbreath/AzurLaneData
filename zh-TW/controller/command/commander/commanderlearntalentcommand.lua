...
function slot0.execute(slot0, slot1)
	...
	if getProxy(PlayerProxy):getData().gold < CommanderTalent.New({}):getConfig("cost") then
		pg.TipsMgr.GetInstance():ShowTips(i18n("common_no_resource"))

		return
	end

	slot17.commanderid = slot3
	slot17.targetid = slot4
	slot17.replaceid = slot5

	pg.ConnectionMgr.GetInstance():Send(25012, {}, 25013, function (slot0)
		if slot0.result == 0 then
			...
			if slot1:getSameGroupTalent(uv4.groupId) then
