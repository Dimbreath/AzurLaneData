...
function slot0.execute(slot0, slot1)
	...
	slot9.id = slot2.id
	slot9.group_id_list = slot2.group_id_list
	slot9.formation_list = slot2.formation_list
	slot9.friend_id = slot2.friend_id
	slot9.friend_ship_id = slot2.friend_ship_id
	slot9.guild_id = slot4.id

	pg.ConnectionMgr.GetInstance():Send(61002, {}, 61003, function (slot0)
		if slot0.result == 0 then
			getProxy(ChapterProxy):getGuildChapter():update(slot0.current_guild)
