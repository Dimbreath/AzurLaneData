...
function slot0.execute(slot0, slot1)
	...
	slot3 = pg.SdkMgr.GetInstance():GetChannelUID()
	slot3 = pg.SDKMgr:GetInstance():GetChannelUID()

	if not slot1:getBody().arg4 then
		slot2.arg4 = "0"
	end
	...
	pg.ConnectionMgr.GetInstance():Connect(NetConst.GATEWAY_HOST, NetConst.GATEWAY_PORT, function ()
		pg.ConnectionMgr.GetInstance():Send(10020, {
			...
			print("disconnect from gateway...", slot0.result)
			...
			if slot0.result == 0 then
				...
				elseif pg.SDKMgr:GetInstance():isPlatform() then
					pg.TipsMgr:GetInstance():ShowTips("登錄錯誤碼：" .. slot0.result)
				else
