...
function slot0.doCellFlagUpdate(slot0)
	...
	if #slot0.data.cell_flag_list > 0 then
		_.each(slot1.cell_flag_list, function (slot0)
			...
			if slot1:getChapterCell(slot0.pos.row, slot0.pos.column) then
...
function slot0.doShipUpdate(slot0)
	...
	if #slot0.data.ship_update > 0 then
		_.each(slot1.ship_update, function (slot0)
			...
			if slot1:getShip(slot0.id) and slot1.hpRant * slot0.hp_rant == 0 and slot1:getShipType() == ShipType.WeiXiu then
...
function slot0.doMove(slot0)
	...
	if #slot0.data.move_path > 0 then
		...
		slot6.row = slot1.arg1
		slot6.column = slot1.arg2
		slot7, slot8 = slot3:findPath(ChapterConst.SubjectPlayer, slot0.chapter.fleet.line, {})
		...
		slot10.row = slot9.row
		slot10.column = slot9.column
		slot4.line = {}
	end
...
function slot0.doOpenBox(slot0)
	...
	if pg.box_data_template[slot6.attachmentId].type == ChapterConst.BoxStrategy then
		...
		slot12.type = DROP_TYPE_STRATEGY
		slot12.id = slot8
...
function slot0.doAmbush(slot0)
	if slot0.op.arg1 == 1 then
		...
		slot6.ShowTips(slot6, slot5.flag == 0 and i18n("chapter_tip_aovid_failed") or i18n("chapter_tip_aovid_succeed"))
	end
end
...
function slot0.doRepair(slot0)
	...
	slot2, slot3, slot9.gem = ChapterConst.GetRepairParams()
...
function slot0.doBarrier(slot0)
	...
	if slot0.chapter:getChapterCell(slot2.arg1, slot2.arg2).attachment ~= ChapterConst.AttachBox or slot4.attachmentId ~= slot6 then
...
function slot0.doTeleportSub(slot0)
	...
	slot5.row = slot1.arg1
	slot5.column = slot1.arg2
	slot3.line = {}
	slot5.row = slot1.arg1
	slot5.column = slot1.arg2
	slot3.startPos = {}
	slot5[1] = slot3.startPos
	slot5[2] = slot3.line
	slot0.fullpath = {}
end
