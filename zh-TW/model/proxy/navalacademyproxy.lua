...
function slot0.register(slot0)
	...
	slot0:on(22001, function (slot0)
		...
		uv0.skillClassNum = slot0.skill_class_num or 2
...
function slot0.isResourceFieldUpgradeConditionSatisfy(slot0)
	...
	if not slot0:GetOilVO():CanUpgrade(slot2.level, slot2.gold) then
		if not slot0:GetGoldVO():CanUpgrade(slot2.level, slot2.gold) then
			if slot0:GetClassVO():CanUpgrade(slot2.level, slot2.gold) then
				return true
			end
		end
	end
