...
function slot0.returnPhaseColor(slot0)
	return slot0.phaseColor[slot0.phase][1] / 255, slot0.phaseColor[slot0.phase][2] / 255, slot0.phaseColor[slot0.phase][3] / 255
end
...
function slot0.setInfomation(slot0, slot1, slot2, slot3, slot4)
	...
	for slot8 = 1, 4, 1 do
		updateDrop(slot0.bonusWindow:Find("window/scrollview/item/award"), slot9)
	...
	if slot0.enterStage then
		function slot2()
			if slot0.isdead then
				return slot0.stageList[0]
			end

			for slot3, slot4 in ipairs(slot0.stageList) do
				if slot0.bossHP < slot4[1][1] + ((slot3 == 1 and 1) or 0) and slot4[1][2] <= slot0.bossHP then
					return slot4[2]
				end
			end
		end
...
function slot0.init(slot0)
	...
	function slot0.fleetEditPanel.onCombat()
		slot0:commitEdit()
		slot0:emit(ActivityBossBattleMediator2.ON_PRECOMBAT, function ()
			if slot0.isdead then
				return slot0.stageList[0]
			end
...
function slot0.enterAnim(slot0)
	function slot1()
		if slot0.titleEffect then
			setActive(slot0.titleEffect, false)
		end

		setActive(slot0.titleTF, true)
	...
	function slot2()
		slot0 = slot0.titleEffect:GetComponent("DftAniEvent")
		...
		slot0:SetTriggerEvent(function (slot0)
			...
			slot0.anim:Play("enter", -1, 0)
		end)
		setActive(slot0.titleEffect, true)
	end

	if not slot0.titleEffect then
		PoolMgr.GetInstance():GetUI("ASKS", true, function (slot0)
			slot0.titleEffect = slot0

			setParent(slot0.titleEffect, slot0._tf)
			setParent()
			onButton(slot0, slot0.titleEffect, function ()
				slot0()
			end)
		end)
	else
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.formationBtn, function ()
		slot0.fleetEditPanel:attach(slot0.fleetEditPanel.attach)
		slot0.fleetEditPanel.attach.fleetEditPanel:setFleets(slot0.fleet)
		slot0.fleetEditPanel.attach.fleetEditPanel.setFleets.fleetEditPanel:set(1, 1)
		pg.UIMgr.GetInstance():BlurPanel(slot0.fleetEditPanel._tf)
	end, SFX_PANEL)
	onButton(slot0, slot0.rankTF, function ()
		slot0:emit(ActivityBossBattleMediator2.ON_RANK)
	end, SFX_PANEL)
...
function slot0.checkStory(slot0)
	...
	slot3 = coroutine.create(function ()
		for slot3, slot4 in ipairs(ipairs) do
			if (slot1.bossHP < slot4[1] + ((slot3 == 1 and 1) or 0) or (slot4[1] == 0 and slot1.isdead)) and not slot1.player:IsPlayed(slot4[2]) then
				slot2:Play(slot4[2], slot3)
				coroutine.yield()
			end
		end
	end)
...
function slot0.updatePage(slot0)
	...
	setImageColor(slot0:findTF("back_light", slot0.mainTF), Color.New(slot0:returnPhaseColor()))

	for slot7 = 1, 4, 1 do
		setSlider(slot0:findTF("Slider", slot8), 0, 2500, math.min(math.max(slot0.bossHP - (slot7 - 1) * 2500, 0), 2500))
		setActive(slot0:findTF("mood", slot0:findTF("phase" .. slot7, slot0.hpBar)), slot7 == slot0.phase)
...
function slot0.showAwards(slot0)
	...
	if not slot0.awardList then
		...
		slot0.awardList:make(function (slot0, slot1, slot2)
			slot3 = slot0.taskList[slot1 + 1][1]
			slot5 = slot0.taskProxy:getTaskById(slot0.taskList[slot1 + 1][2]) or slot0.taskProxy:getFinishTaskById(slot4) or Task.New({
				id = slot4
			})

			if slot0 == UIItemList.EventInit then
				setText(slot2:Find("mask/target/Text"), i18n("world_boss_award_limit", string.format("%2d", slot3 / 100)))
			...
			if slot0 == UIItemList.EventUpdate then
				...
				setActive(slot2:Find("award/mask_can"), slot0.bossHP <= slot3 and slot5:isFinish() and not slot5:isReceive())
			end
		end)
	end
...
function slot0.updateTaskBtn(slot0, slot1, slot2, slot3)
	...
	onButton(slot0, slot4[2], function ()
		slot0:emit(ActivityBossBattleMediator2.ON_GET, slot1.id)
	end, SFX_PANEL)
end
...
function slot0.openCommanderPanel(slot0, slot1)
	...
	slot0.commanderFormationPanel:update(slot1, function (slot0)
		slot0:emit(ActivityBossBattleMediator2.ON_SELECT_COMMANDER, slot1.id, slot0)
		slot0:hideFleetEdit()
	end)
...
function slot0.addMsg(slot0, slot1)
	...
	setParent(slot3:GetChild(0), slot2)
end
...
function slot0.setRank(slot0, slot1, slot2)
	...
	for slot7 = 1, 3, 1 do
		...
		if slot1[slot7] then
			...
			GetImageSpriteFromAtlasAsync("SquareIcon/" .. slot9, "", slot3:Find("icon"))
