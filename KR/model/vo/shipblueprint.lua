...
slot0.Ctor = function (slot0, slot1)
	...
	slot0.state = slot0.STATE_LOCK
	slot0.startTime = 0
	slot0.shipId = 0
	slot0.duration = 0
	slot0.strengthenConfig = {}

	for slot5, slot6 in ipairs(slot0:getConfig("strengthen_effect")) do
		if Clone(slot1[slot6]).special == 1 then
	...
	for slot5, slot6 in ipairs(slot0:getConfig("fate_strengthen")) do
		if Clone(slot1[slot6]).special == 1 then
			slot0:warpspecialEffect(slot7)
		end
...
slot0.warpspecialEffect = function (slot0, slot1)
	...
	if slot1.effect_breakout ~= 0 then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_BREAKOUT,
	...
	if type(slot1.change_skill) == "table" then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_CHANGE_SKILL,
			slot1.change_skill,
			slot3[slot4 + 1] or ""
		})
	...
	if type(slot1.effect_base) == "table" then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_BASE_LIST,
			slot1.effect_base,
			slot3[slot4 + 1] or ""
		})
	...
	if type(slot1.effect_preload) == "table" then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_PRLOAD_COUNT,
	...
	if type(slot1.effect_dialog) == "table" then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_DIALOGUE,
			slot1.effect_dialog,
			slot3[slot4 + 1] or ""
		})
	...
	if type(slot1.effect_equipment_proficiency) == "table" then
		table.insert(slot2, {
			slot0.STRENGTHEN_TYPE_EQUIPMENTPROFICIENCY,
...
slot0.updateState = function (slot0)
	if slot0:isFetched() then
		slot0.state = slot0.STATE_UNLOCK
	elseif slot0.startTime == 0 then
		slot0.state = slot0.STATE_LOCK
	elseif slot0:isFinishedAllTasks() then
		slot0.state = slot0.STATE_DEV_FINISHED
	else
		slot0.state = slot0.STATE_DEV
	end
end
...
slot0.isLock = function (slot0)
	return slot0.state == slot0.STATE_LOCK
end

slot0.isDeving = function (slot0)
	return slot0.state == slot0.STATE_DEV
end

slot0.isFinished = function (slot0)
	return slot0.state == slot0.STATE_DEV_FINISHED
end
...
slot0.isUnlock = function (slot0)
	return slot0.state == slot0.STATE_UNLOCK
end
...
slot0.isFinishedAllTasks = function (slot0)
	...
	return _.all(slot0:getTaskIds(), function (slot0)
		return slot0:getTaskStateById(slot0) == slot1.TASK_STATE_FINISHED
	end)
end
...
slot0.getTaskStateById = function (slot0, slot1)
	if slot0:isLock() then
		if slot0.duration > 0 then
			return slot0.TASK_STATE_PAUSE
		else
			return slot0.TASK_STATE_LOCK
		end
	elseif pg.TimeMgr.GetInstance():GetServerTime() < slot0:getTaskOpenTimeStamp(slot1) then
		return slot0.TASK_STATE_WAIT
	elseif getProxy(TaskProxy):getTaskVO(slot1) and slot5:isReceive() then
		return slot0.TASK_STATE_FINISHED
	elseif slot5 and slot5:isFinish() then
		return slot0.TASK_STATE_ACHIEVED
	elseif slot5 then
		return slot0.TASK_STATE_START
	else
		return slot0.TASK_STATE_OPENING
	end
end
	...
	for slot5 = 1, slot0.level, 1 do
		if slot0.strengthenConfig[slot5].special == 1 and type(slot6.special_effect) == "table" then
			for slot10, slot11 in ipairs(slot6.special_effect) do
				if slot11[1] == slot0.STRENGTHEN_TYPE_ATTR then
					slot1[slot11[2][1]] = (slot1[slot11[2][1]] or 0) + slot12[2]
				end
			end
		end
	end

	for slot5 = 1, slot0.fateLevel, 1 do
...
slot0.getUseageMaxItem = function (slot0)
	...
	for slot5 = slot0.level + 1, slot0:getMaxLevel(), 1 do
...
slot0.getFateUseageMaxItem = function (slot0)
	...
	for slot5 = slot0.fateLevel + 1, slot0:getMaxFateLevel(), 1 do
	...
	for slot5 = 1, slot0.level, 1 do
		if slot0:getStrengthenConfig(slot5).special == 1 and type(slot6.special_effect) == "table" then
			for slot11, slot12 in ipairs(slot7) do
				if slot12[1] == slot0.STRENGTHEN_TYPE_DIALOGUE then
...
slot0.getUnlockLevel = function (slot0, slot1)
	for slot6 = 1, slot0:getMaxLevel(), 1 do
		if type(slot0:getStrengthenConfig(slot6).special_effect) == "table" then
			for slot12, slot13 in ipairs(slot8) do
				if slot13[1] == slot0.STRENGTHEN_TYPE_DIALOGUE then
...
slot0.getBaseList = function (slot0, slot1)
	for slot5 = slot0.level, 1, -1 do
		if slot0:getStrengthenConfig(slot5).special == 1 then
			for slot11, slot12 in ipairs(slot7) do
				if slot12[1] == slot0.STRENGTHEN_TYPE_BASE_LIST then
					return slot12[2]
				end
			end
		end
...
slot0.getPreLoadCount = function (slot0, slot1)
	for slot5 = slot0.level, 1, -1 do
		if slot0:getStrengthenConfig(slot5).special == 1 then
			for slot11, slot12 in ipairs(slot7) do
				if slot12[1] == slot0.STRENGTHEN_TYPE_PRLOAD_COUNT then
					return slot12[2]
				end
			end
		end
...
slot0.getEquipProficiencyList = function (slot0, slot1)
	...
	for slot6 = 1, slot0.level, 1 do
		if slot0:getStrengthenConfig(slot6).special == 1 then
			for slot12, slot13 in ipairs(slot8) do
				if slot13[1] == slot0.STRENGTHEN_TYPE_EQUIPMENTPROFICIENCY then
...
slot0.isFinishPrevTask = function (slot0, slot1)
	slot3 = nil
