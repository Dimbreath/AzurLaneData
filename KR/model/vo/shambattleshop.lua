slot0 = class("ShamBattleShop", import(".BaseVO"))
...
function slot0.isOpen(slot0)
	...
	if slot0:bindConfigTable()[slot0.id] and pg.TimeMgr.GetInstance():STimeDescS(pg.TimeMgr.GetInstance().GetServerTime(slot3), "*t").month == slot0.id then
		return slot2.sim_shop_time[1] <= slot4.day and slot4.day <= slot2.sim_shop_time[2]
	end
end
...
function slot0.getRestDays(slot0)
	...
	if slot0:bindConfigTable()[slot0.id] then
		slot1 = slot2.sim_shop_time[2] - pg.TimeMgr.GetInstance():STimeDescS(pg.TimeMgr.GetInstance().GetServerTime(slot3), "*t").day + 1
	end
