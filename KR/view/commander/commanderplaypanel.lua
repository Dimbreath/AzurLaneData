...
function slot0.updateSkillTF(slot0, slot1)
	...
	if slot4:isMaxLevel() then
		...
		slot0.expTxt.text = slot4.exp .. ((slot1 == 0 and "") or "<color=#92fc63FF>(+" .. slot1 .. ")</color>") .. "/" .. slot4:getNextLevelExp()
	end
...
function slot0.updateCommanderTF(slot0, slot1)
	...
	if slot0.commanderVO:isMaxLevel() then
		...
		slot0.sliderExpTxt.text = slot2.exp .. ((slot1 > 0 and "<color=#92fc63FF>(+" .. slot1 .. ")</color>") or "") .. "/" .. slot2:getNextLevelExp()
